...
-record(gm_role_info, {gs_system_role_info, 
			   ...
			   guildtype,			%%帮会类型
			   honor,				%%荣誉值
			   battlefield_dead_counter,		% 战场死亡次数([{BattleFieldId, Counter}, .......])
			   pet_skill,
			   money_check_lock
		       }).
...
set_roleinfo(RoleInfo,Role_id,Role_Class,Gender,Role_Level,RoleState,Role_pid,Role_node,Role_pos,
						...
						Hitrate,Stamina,Agile,Strength,Intelligence,RoleIimmunes,RoleDebuffimmunes,RoleDefenses,
						GuildName,GuildPosting,RoleBuffs,Viptag,AllIcons,Crime,Pkmodel,Path,GS_GateInfo,
						GS_MapInfo,RoleServerId,Treasure_Transport,Fighting_force,Honor,Spouse,
						Pet_skill,MoneyCheckLock)->
		RoleInfo#gm_role_info{
				...
				spouse = Spouse,
				pet_skill = Pet_skill,
				money_check_lock = MoneyCheckLock
			}.
...
get_dancing_role_from_roleinfo(RoleInfo)->
	#gm_role_info{dancing_role=Dancing_role} = RoleInfo,
	Dancing_role.
set_dancing_role_to_roleinfo(RoleInfo,Dancing_role) ->
	RoleInfo#gm_role_info{dancing_role=Dancing_role}.	

get_spouse_from_roleinfo(RoleInfo)->
	#gm_role_info{spouse=Spouse} = RoleInfo,
	Spouse.
set_spouse_to_roleinfo(RoleInfo,Spouse) ->
	RoleInfo#gm_role_info{spouse=Spouse}.	
...
get_pet_skill_from_roleinfo(RoleInfo)->
	#gm_role_info{pet_skill=Value} = RoleInfo,
	Value.
	
set_pet_skill_to_roleinfo(RoleInfo,Value) ->
	RoleInfo#gm_role_info{pet_skill=Value}.

get_money_check_lock_from_roleinfo(RoleInfo)->
	#gm_role_info{money_check_lock=Value} = RoleInfo,
	Value.
	
set_money_check_lock_to_roleinfo(RoleInfo,Value) ->
	RoleInfo#gm_role_info{money_check_lock=Value}.
...
% role进程state
-record(role_state, {
		pet_shop_end_time = 0,			% 宠物商店本论刷新结束时间
		pet_shop_goods_list = [],		% 宠物商店本论刷新商品
		buy_pet = [],					% 宠物商店本轮已经购买的宠物
		timer_ref = [],					% 计时器引用
		singlehood_get_prize_time		% 光棍节每日领取时间
	}).
