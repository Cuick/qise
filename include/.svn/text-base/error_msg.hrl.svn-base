...
-define(ERROR_LESS_SILVER, 10062).		%%铜钱不够
-define(MONEY_CHANGE_ERROR,10076).      %%扣元宝错误 
-define(ERROR_NOT_LEAVE_ATTACK,10077).	%%尚未脱离战斗
-define(SKILL_LEVEL_MAX, 10078).
-define(ERROR_UNKNOWN,10003).				%%未知错误
-define(ERROR_COUNT_RESTRICT,11574).
...
-define(ERROR_ACTIVITY_COOLTIME_DANCING_ERR,10766).	%%搓澡未冷却
-define(ERROR_ACTIVITY_COOLTIME_CHEERING_ERR,10767).	%%戏水未冷却
-define(ERROR_BANQUET_CAN_NOT_TOUCH_DANCING_ERR,10768).	%%不能对她跳舞了
-define(ERROR_BANQUET_CAN_NOT_TOUCH_CHEERING_ERR,10769).	%%不能对她干杯了
-define(ERROR_BANQUET_TOUCH_LIMIT_ERR,10770).				%%次数限制
-define(ERROR_ACTIVITY_COOLTIME_SWANKING_ERR,10771).
...
-define(PET_NAME_OK,10804).						%%宠物名ok
...
-define(ERROR_PET_CANNOT_FIND_SKILL,10866).		%%can't find that skill
...
%%跳舞未在视野
-define(DANCING_ERROR_NO_ROLE_INAOI,11111).
...
%%Banquet
-define(ERRNO_CAN_NOT_DO_IN_BANQUET,11446).		%%正在泡澡,无法进行本次操作
...
%%新宠物模块
-define(ERROR_PET_GRADE_UP_TO_TOP,13001).               %%宠物位阶已达到上限
-define(ERROR_PET_GRADE_UP_OK,13002).			        %%宠物位阶提升成功
-define(ERROR_PET_GRADE_UP_FAILED,13003).		        %%宠物位阶提升失败
-define(ERROR_PET_GRADE_UP_EXTITEMS_FAIL,13004).        %%位阶升级额外物品进阶符足
-define(ERROR_PET_QUALITY_RISEUP_INIT_FAILED,13005).	%%宠物品阶初始化失败
-define(ERROR_PET_QUALITY_RISEUP_TO_TOP,13006).			%%宠物品质已达到上限
-define(ERROR_PET_QUALITY_RISEUP_FAIL,13007).			%%宠物品质提升失败
-define(ERROR_PET_QUALITY_RISEUP_OK,13008).		        %%宠物品质提升成功
-define(ERROR_PET_SAVVY_UP_FAIL, 13009).                %%宠物悟性升级失败
-define(ERROR_PET_SAVVY_UP_OK, 13010).                  %%宠物悟性升级成功
-define(ERROR_PET_SAVVY_UP_TOP, 13011).                 %%宠物悟性到达上限
-define(ERROR_PET_QUALIFICATION_MAX, 13014).            %%宠物资质已经是最大值
-define(ERROR_PET_QUALIFICATION_SUCCESS, 13015).        %%宠物资质提升成功
-define(ERROR_PET_QUALIFICATION_FAILED, 13016).         %%宠物资质提升失败
-define(ERROR_GRADE_EXTITEMS_ERROR, 13017).             %%宠物位阶提升额外道具传送值错误
-define(ERROR_GRADEPET_NOEXIST, 13018).                 %%宠物位阶初始化信息
-define(ERROR_QUALITY_GRADE_MAX, 13019).				% 宠物资质进阶已达最大
-define(ERROR_PET_SHOP_EMPTY, 13020).					% 本轮宠物已买光
-define(ERROR_PET_SHOP_NOT_EXIST, 13021).				% 宠物不再本轮商店中出售
-define(ERROR_PET_TYPE_SAME, 13022).					% 转换类型与原类型一致
-define(ERROR_PET_TYPE_CHANGE_PROP_NOT_ENOUGH, 13023).					% 转换道具不足
-define(ERROR_PET_TYPE_ATTR_LOCK_REPEAT, 13024).					% 锁定属性重复
-define(ERROR_PET_TYPE_ATTR_NOT_ENOUGH, 13025).					% 精炼道具不足
-define(ERROR_PET_TYPE_ATTR_NOT_EXIST, 13026).					% 属性不存在
-define(ERROR_PET_TYPE_ATTR_NOT_ALL_LOCK, 13027).					% 不可全部锁定
-define(ERROR_PET_INHERIT_SAME, 13028).					% 主副宠物相同

-define(ERROR_HAD_REWARDED_CHARGE_PACKAGE, 14001).		% 6重礼包已领取

-define(ERROR_EQUIPMENT_MOVE_QUALITY, 14002).               % 强化转移装备品质有误
-define(ERROR_EQUIPMENT_MOVE_PROP_NOT_ENOUGH, 14003).       % 强化转移道具不足
-define(ERROR_NOT_HOLIDAY, 14004).							% 近期无活动
-define(ERROR_HAVE_GET_HOLIDAY_PRIZE, 14005).				% 今日活动福利已经领取
-define(ERROR_JOIN_BATTLE, 14006).							% cd时间未到,少后再试
-define(ERROR_ANSWER_PROP, 14007).							% 该功能已经用完

-define(ERROR_ROLE_OFFLINE, 14100).                         % role offline

-define(ERROR_SYSTEM, 99999).                              % 系统错误

%% 专题活动
-define(ERROR_PACKAGE_FULL, 15001).                           %% 背包已满
-define(ERROR_ACTIVITY_OVERDUE, 15002).                       %% 活动已过期
-define(ERROR_ACTIVITY_CONDITION_NOT_REACH, 15003).           %% 活动条件未达成
-define(ERROR_ACTIVITY_HAVE_AWARDED, 15004).                  %% 活动奖励已领取
-define(ERROR_ACTIVITY_REWARD_OVERDUE, 15005).                %% 活动奖励已过期

%% 邀请好友
-define(ERROR_INVITE_FRIENDS_NO_MATCH_GIFTS, 618010).         %% 没有匹配的礼物
-define(ERROR_INVITE_FRIENDS_HAD_REWARD, 618012).             %% 礼物已领取
-define(ERROR_INVITE_FRIENDS_CONDITION_NOT_REACH, 618013).    %% 领取条件不足

% 升级送元宝
-define(ERROR_GETGOLDBYLEVEL_FAIL, 16000).  % 失败

% 
-define(ERRNO_NOT_DO_IN_THIS_MAP, 16001).                    %% 本地徒步能骑马

%% travel battle

-define (TRAVEL_BATTLE_NOT_START, 16002).                        %% 跨服战未开始
-define (TRAVEL_BATTLE_SYSTEM_ERROR, 16004).                     %% 跨服战分区错误
-define (TRAVEL_BATTLE_SERVER_PERSON_MAX, 16005).                %% 跨服战当前分区人数已到最大值
-define (TRAVEL_BATTLE_MATCH_OK, 16006).                         %% 跨服战报名后匹配成功
-define (TRAVEL_BATTLE_NOT_REGISTERED, 16007).                   %% 跨服战未报名
-define (TRAVEL_BATTLE_HAS_REGISTERED, 16008).                   %% 跨服战已报名
-define (TRAVEL_BATTLE_STAGE_ERROR, 16009).                      %% 跨服战场次错误
-define (TRAVEL_BATTLE_CONDITION_NOT_MATCH, 16010).              %% 跨服战报名条件不符
-define (TRAVEL_BATTLE_REGISTER_CD, 16011).                      %% 跨服战报名CD中
-define (TRAVEL_BATTLE_SHOP_ITEM_NOT_EXIST, 16012).              %% 跨服战商店物品不存在
-define (TRAVEL_BATTLE_SHOP_CLOSED, 16013).                      %% 跨服战商店关闭
-define (TRAVEL_BATTLE_SCORES_NOT_ENOUGH, 16014).                %% 跨服战积分不足
-define (TRAVEL_BATTLE_FREE_RIGISTER_TICKET, 16015).             %% 跨服战因有礼券导致无免费报名次数奖励
-define (TRAVEL_BATTLE_NO_FREE_TIMES, 16016).                    %% 跨服战免费报名次数已用完
-define (TRAVEL_BATTLE_HAS_ENTERED_MAP, 16017).                  %% 跨服战已进入跨服地图
-define (TRAVEL_BATTLE_NOT_ENTER_MAP, 16018).                    %% 跨服战未进入跨服地图
-define (TRAVEL_BATTLE_RANK_DATA_NOT_AVAILABLE, 16019).          %% 跨服战排名数据不可获取
-define (TRAVEL_BATTLE_SESSION_ERROR, 16020).                    %% 跨服战届次错误
-define (TRAVEL_BATTLE_PAGE_ERROR, 16021).                       %% 跨服战请求分页错误
-define (TRAVEL_BATTLE_IN_PROGRESS, 16022).                      %% 跨服战进行中
-define (TRAVEL_BATTLE_AWARDS_MAIL, 16023).                      %% 跨服战奖励通过邮件发放
-define (TRAVEL_BATTLE_DATA_NOT_AVAILABLE, 16024).               %% 跨服战数据无法获取
-define (TRAVEL_BATTLE_INVALID_OPERATION, 16025).                %% 跨服战场景影响战力操作不允许

%% open charge feedback
-define (OPEN_CHARGE_FEEDBACK_OVERDUE, 16030).
-define (OPEN_CHARGE_FEEDBACK_AWARDS_MAIL, 16031).

%% open service auction
-define (OPEN_SERVICE_AUCTION_OVERDUE, 16032).
-define (OPEN_SERVICE_AUCTION_GOLD_NOT_ENOUGH, 16033).
-define (OPEN_SERVICE_AUCTION_BID_TWICE, 16034).
-define (OPEN_SERVICE_AUCTION_GOLD_LESS_BASE, 16035).
-define (OPEN_CHARGE_FEEDBACK_LOTTERY_ERROR, 16036).

%% wedding
-define(ERROR_WEDDING_ROLE_REFUSED, 16100).                  %% wedding object refused
-define(ERROR_WEDDING_ROLE_HAS_MARRIED, 16101).              %% wedding role married
-define(ERROR_WEDDING_WRONG_CEREMONY_TIME, 16102).
-define(ERROR_WEDDING_ROLE_NOT_MARRIED, 16103).


-define(ERRNO_GO_TO_BANLENCE, 17000).  
...
% 积分不够  18000
% 兑换物品不存在	18001
-define(ERR_PET_SLOT_FULL, 19000). 		%%技能槽位已满
-define(ERR_PET_SLOT_OPEND, 19001). 		%%技能槽位已开启
-define(ERR_PET_HEART_LOW, 22000). 		%%宠物心情值不够，无法出战
-define(HANENOT_SLOT, 19002).			%%没有可用技能槽


-define(NO_PET_OUT, 19003).

-define(LESS_PET_SKILL_LOCK, 19004).

-define(ERROR_PET_SAME_TYPE, 19005). %类型相同，不能技能融合

-define(ERR_IN_HOTEL, 19006). %%宠物在客栈中，无法出战

-define (ERROR_TEMP_ACTIVITY_AWARD_SEND_TYPE, 16037).
-define (ERROR_TEMP_ACTIVITY_OVERDUE, 16038).
-define (ERROR_TEMP_ACTIVITY_CONDITION_CHECK_FAILED, 16039).

%% travel match
-define (TRAVEL_MATCH_NOT_REGISTER_TIME, 20000).                    %% 非报名时间
-define (TRAVEL_MATCH_SYSTEM_ERROR, 20001).                         %% 系统错误
-define (TRAVEL_MATCH_REGISTER_TWICE, 20002).                       %% 重复报名
-define (TRAVEL_MATCH_NOT_REGISTER, 20003).                         %% 未报名
-define (TRAVEL_MATCH_STAGE_FAILED, 20004).                         %% 已淘汰
-define (TRAVEL_MATCH_NOT_WAIT_MAP_TIME, 20005).                    %% 不可以进入等待地图时间
-define (TRAVEL_MATCH_PLAYER_LIST_NOT_AVAILIABLE, 20006).           %% 无法获取分组列表
-define (TRAVEL_MATCH_RANK_DATA_NOT_AVAILIABLE, 20007).             %% 无法获取分届排名数据
-define (TRAVEL_MATCH_LEVEL_COUNT_LIMIT, 20008).                    %% 等级段报名人数达到上限

-define(SINKER_NOT_ENOUGH,4001).               %%砸金蛋 锤子不够
-define(ERROR_ALREADY_SMASHED,4002).           %%砸金蛋 已经砸过了

-define(GOD_TREE_GOLD_NOT_ENOUGH,4003).        %%七色神树 元宝不够
-define(GOD_TREE_NOT_START,4004).              %%七色神树 还没有开始
-define(GOD_TREE_PACKET_NOT_ENOUGH,4005).      %%七色神树 仓库空间不足

-define(DEAD_VALLEY_SYSTEM_ERROR, 21000).                    %% 系统错误
-define(DEAD_VALLEY_ALL_ZONE_OVERLOAD, 21001).               %% 人数已满
-define(DEAD_VALLEY_NOT_START, 21002).                       %% 未开始
-define(DEAD_VALLEY_ZONE_ERROR, 21003).                      %% 人已在场景内
-define(DEAD_VALLEY_NO_DROP_EQUIPMENT, 21004).               %% 无可掉落的装备
-define(DEAD_VALLEY_LESS_POINTS, 21005).                     %% 捡宝值不足
-define(DEAD_VALLEY_COOLDOWN_TIME, 21006).                   %% CD中
-define(PLEASE_LEAVE_VIPMAP, 21007).                   %% please leave vip map

-define(ERROR_MONEY_NOT_ENOUGH,21008).
