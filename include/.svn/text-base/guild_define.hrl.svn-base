...
-define(GUILD_JOIN_RESTICT_TIME,5*60).					%%5小时, 改为5分钟
...
-define(GUILD_MIN_LEVEL_CREATE,25).								%%建立帮会的最小级别
-define(GUILD_MIN_LEVEL_JOIN,25).								%%加入帮会最小级别
...
-define(UPDATE_MEMBERINFO_TO_CLIENT_INTERVAL,3000).	%%更新帮会成员信息给客户端的时间间隔
...
% 帮会人员信息更新存储结构
-record(update_guild_member_msg, {
		guild_id,				% 帮会id
		role_id_list			% 帮会成员角色id
	}).
