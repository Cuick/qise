...
-define(LEVEL_LOST_LIST,level_lost_list).
		...
		true->
			...
			case can_challenge_rank(Level) of										
				true ->																
					...
					put(?LEVEL_LOST_LIST, NewLostList),
		...
		case role_pos_util:is_role_online(RoleId) of
			true ->
				role_pos_util:send_to_role(RoleId, 
					...
					?TITLE_TYPE_LEVEL, Rank});
			false ->
				title_op:hook_on_condition_change_offline(
					?TITLE_TYPE_LEVEL, RoleId, Rank)	
		end
	end,
	lists:foreach(fun(RoleId) ->
		Fun(RoleId, -1)
	end, get(?LEVEL_LOST_LIST)),
	lists:foldl(fun({RoleId, _, _}, Rank) ->
		Fun(RoleId, Rank),
		Rank + 1
	end, 1, get(?LEVEL_TOP_LIST)).