...
init()->
	...
	AllRoleRankInfo = lists:foldl(fun({{Type,RoleId},_,_},RankInfoTmp)->
						if
							(Type =:= ?RANK_TYPE_CHESS_SPIRITS_TEAM) or (Type =:= ?RANK_TYPE_PET_TALENT_SCORE) or is_tuple(Type)->
								RankInfoTmp;
							true ->
	...
	Flag = travel_battle_util:is_travel_battle_server(),
	if
		Flag ->
			nothing;
		true ->
...
get_rank(ActivityId,Type)->
	erlang:apply(get_module_by_type(Type),get_rank, [ActivityId]).
...
get_role_base_info_from_db(RoleId)->
	case role_db:get_role_info(RoleId) of
		...
		RoleInfo ->
			...
			RoleServerId = travel_battle_util:get_serverid_by_roleid(RoleId), 
			GuildName = 
