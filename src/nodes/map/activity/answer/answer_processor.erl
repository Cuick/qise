...
-export([start_link/2,apply_join_activity/1,apply_answer_question/1,get_activity_state/0,
		 apply_leave_activity/1, get_answer_info/1]).
...
apply_join_activity(Info)->
	try
		...
		{Double,Auto} = answer_op:get_vip_prop_count(VipProps),
		global_util:call(?MODULE,{apply_join_activity,Info, {Double,Auto}})
	catch
		E:R->
...
get_answer_info(RoleId)->
	try
		global_util:call(?MODULE,{get_answer_info, RoleId})
	catch
		E:R->
			% slogger:msg("answer get_activity_state error ~p ~p ~n ",[E,R]),
			error
...
%% --------------------------------------------------------------------
handle_call({apply_join_activity,Info, {Double,Auto}}, _From, State) ->
	Reply = 
	try
		answer:apply_join_activity(Info, {Double,Auto})
	catch
		E:R->
...
handle_call({get_answer_info, Info}, _From, State) ->
	Reply = 
	try
		answer:get_answer_info(Info)
	catch
		E:R->
			slogger:msg("answer apply_answer_question error ~p ~p ~p ~n ",[E,R,erlang:get_stacktrace()]),
			error
