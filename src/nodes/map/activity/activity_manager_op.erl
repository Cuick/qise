...
on_check()->
	try
		travel_battle_util:do_in_not_share_node(answer_manager_op,on_check,[])
	catch
		E_ANSWER:R_ANSWER->
	...
	try
		travel_battle_util:do_in_not_share_node(banquet_manager_op,on_check,[])
	catch
		E_SPA:R_SPA->
			slogger:msg("banquet_manager_op:on_check() ~p ~p,~n~p~n",
						[E_SPA,R_SPA,erlang:get_stacktrace()]),
			error
	end,
	try
		travel_battle_util:do_in_not_share_node(?MODULE,battle_check,[])
	catch
		E_BATTLE:R_BATTLE->
	...
	try
		travel_battle_util:do_in_not_share_node(dragon_fight_manager,on_check,[])
	catch
		E_DRAGON:R_DRAGON->
	...
	try
		travel_battle_util:do_in_not_share_node(treasure_transport_manager,on_check,[])
	catch
		E5:R5->
...
get_activity_state(?BANQUET_ACTIVITY)->
	banquet_manager_op:get_activity_state();
get_activity_state(_)->
	nothing.

apply_join_activity(?BANQUET_ACTIVITY,Info)->
	banquet_manager_op:apply_join_activity(Info);
apply_join_activity(_,_)->
	nothing.

apply_leave_activity(?BANQUET_ACTIVITY,Info)->
	banquet_manager_op:apply_leave_activity(Info);
...
apply_stoped_activity(?BANQUET_ACTIVITY,Info)->
	banquet_manager_op:apply_stop_me(Info);
...
battle_check()->
    ...
    CampBattleInfoList = answer_db:get_activity_info(?CAMP_BATTLE_ACTIVITY),
	CheckFunCamp = fun(Info)->
	    {Type,StartLines} = answer_db:get_activity_start(Info),
	    activity_manager_op:activity_forecast_check(?CAMP_BATTLE_ACTIVITY,Type,StartLines)
    end,
    lists:foreach(CheckFunCamp, CampBattleInfoList),
