...
request_banquetlist(RoleId)->
	try
		global_util:send(?MODULE,{request_banquetlist,RoleId})
	catch
		E:R->
			slogger:msg("request_banquetlist error ~p ~p ~n ",[E,R]),
			error
	end.

banquet_touch_other_role(Type,Info)->
	try
		global_util:send(?MODULE,{banquet_touch_other_role,Type,Info})
	catch
		E:R->
			slogger:msg("banquet_cheering error ~p ~p ~n ",[E,R]),
			error
	end.
banquet_pet_swanking(Type,Info) ->
	try
		global_util:send(?MODULE,{banquet_pet_swanking,Type,Info})
	catch
		E:R->
			slogger:msg("banquet_pet_swanking error ~p ~p ~n ",[E,R]),
			error
	end.

banquet_add_vip_count(RoleId,AddCount)->
	try
		global_util:send(?MODULE,{banquet_add_vip_count,RoleId,AddCount})
	catch
		E:R->
			slogger:msg("banquet_add_vip_count error ~p ~p ~n ",[E,R]),
			error
	end.
...
handle_info({request_banquetlist,RoleId}, State)->
	try
		banquet_manager_op:request_banquetlist(RoleId)
	catch
		E:R->
			slogger:msg("request_banquetlist error ~p ~p ~p ~n ",
						[E,R,erlang:get_stacktrace()]),
			error
	end,
	{noreply, State};

handle_info({banquet_touch_other_role,Type,Info}, State)->
	try
		banquet_manager_op:banquet_touch_other_role(Type,Info)
	catch
		E:R->
			slogger:msg("banquet_cheering error ~p ~p ~p ~n ",
						[E,R,erlang:get_stacktrace()]),
			error
	end,
	{noreply, State};

handle_info({banquet_pet_swanking,Type,Info}, State)->
	try
		banquet_manager_op:banquet_pet_swanking(Type,Info)
	catch
		E:R->
			slogger:msg("banquet_pet_swanking error ~p ~p ~p ~n ",
						[E,R,erlang:get_stacktrace()]),
			error
	end,
	{noreply, State};

handle_info({banquet_add_vip_count,RoleId,AddCount}, State)->
	try
		banquet_manager_op:banquet_add_vip_count(RoleId,AddCount)
	catch
		E:R->
			slogger:msg("banquet_cheering error ~p ~p ~p ~n ",
						[E,R,erlang:get_stacktrace()]),
...
handle_info({banquet_start_notify,InstanceLevel}, State)->
	try
		banquet_manager_op:banquet_start_notify(InstanceLevel)
	catch
		E:R->
			slogger:msg("banquet_start_notify error ~p ~p ~p ~n ",
						[E,R,erlang:get_stacktrace()]),
			error
	end,
	{noreply, State};
