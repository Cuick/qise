...
on_init(ProcName,{Node,GuildA,GuildB})->
	...
	case map_manager:start_instance(MapProc,MapId,instance,
		...
		{atom_to_list(ProcName),?YHZQ_INSTANCEID}) of
		ok->
				...
				false ->
					case lists:keyfind(RoleId,1,get(bluelist)) of
						...
						_->
							...
							gm_logger_role:insert_log_activity(RoleId,6,2,0,0,0,JoinTime,LeaveTime),
							send_camp_info_to_all()
					end;
				_->
					put(redlist,lists:keydelete(RoleId,1,get(redlist))),
					gm_logger_role:insert_log_activity(RoleId,6,1,0,0,0,JoinTime,LeaveTime),
					send_camp_info_to_all()
			end;
		_->
			%%记录在颁奖中 离开的用户
	...
	lists:foreach(fun({RoleId,BaseInfo,KillNum,_})->
						...
						case lists:member(RoleId,FilitList) of
							...
							_->
								...
								role_pos_util:send_to_role(RoleId,{notify_yhzq_reward,get(winner),Honor,RoleLevel*RAddExp,JoinTime,LeaveTime})
						end
					end,get(redlist)),
	lists:foreach(fun({RoleId,BaseInfo,KillNum,_})->
						{_,_,_,RoleLevel} = BaseInfo,
						case lists:member(RoleId,FilitList) of
							...
							_->
								...
								role_pos_util:send_to_role(RoleId,{notify_yhzq_reward,get(winner),Honor,RoleLevel*BAddExp,JoinTime,LeaveTime})
						end
					end,get(bluelist)).
...
%%
creature_killed_score(Killer,BeKilled)->
%%	CreatureInfo = creature_op:get_creature_info(BeKilledNpc),
