...
get_camp_battle_last_record(RoleId)->
	try
		global_util:send(?MODULE,{get_camp_battle_last_record,RoleId})
	catch
		E:R->
			slogger:msg("get_camp_battle_last_record error ~p ~p ~n ",[E,R]),
			error
	end.

get_camp_battle_player_num(RoleId)->
	try
		global_util:send(?MODULE,{get_camp_battle_player_num, RoleId})
	catch
		E:R->
			slogger:msg("get_camp_battle_player_num error ~p ~p ~n ",[E,R]),
			error
	end.
...
handle_info({get_camp_battle_last_record,RoleId},State)->
	try
		battle_ground_manager_op:get_camp_battle_last_record(RoleId)
	catch
		E:R->
			slogger:msg("get_camp_battle_last_record error ~p ~p ~p ~n",[E,R,erlang:get_stacktrace()]),
			error
	end,
	{noreply, State};

handle_info({get_camp_battle_player_num,RoleId},State)->
	try
		battle_ground_manager_op:get_camp_battle_player_num(RoleId)
	catch
		E:R->
			slogger:msg("get_camp_battle_player_num error ~p ~p ~p ~n",[E,R,erlang:get_stacktrace()]),
			error
	end,
	{noreply, State};
