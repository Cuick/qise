...
-define(KILL_HONOR, 1).			% 杀一人获得奖励
-define(TANGLE_MAX_HONOR, 80).	% 群雄逐鹿最高可获得荣誉
-define(ENTER_TANGLE_HONOR, 40).	% 玩家进入群雄逐鹿获得荣誉
...
on_init(ProcName,{BattleType,BattleId})->
	...
	case map_manager:start_instance(MapProc,MapId,instance,
		...
		ok->
			init(Duration + ?TANGLE_BATTLE_BUFFER_TIME_S * 2 * 1000,BattleType,BattleId,MapProc,ProtoInfo),
			ok;
		error->
			on_destroy()
	end.
...
init(Duration,BattleType,BattleId,MapProc,ProtoInfo)->
	...
	put(honor, []),				% 荣誉值([{RoleId, Honor,Node}, .......])
...
add_role_score(RoleId,Score,Type)->
	case lists:keyfind(RoleId,1,get(ranks_info)) of
		false->
			[];
		{_,Name,OriScore,Kills,Time,Node}->
			...
			put(ranks_info,lists:keyreplace(RoleId,1,get(ranks_info),{RoleId,Name,NewScore,NewKills,Time,Node})),
...
get_role_name(RoleId)->
	case lists:keyfind(RoleId,1,get(ranks_info)) of
		false->
			[];
		{RoleId,RoleName,_Ranks,_Kills,_ExtInfo,Node}->
			RoleName
	end.
	
on_role_join({RoleId,RoleName,RoleClass,RoleGender,RoleLevel,Node})->
	...
	case lists:keyfind(RoleId,1,get(ranks_info)) of
		false->
			put(ranks_info,get(ranks_info) ++ [{RoleId,RoleName,0,0,{RoleClass,RoleGender,RoleLevel},Node}]),
			% put(ranks_info,lists:keyreplace(RoleId,1, get(ranks_info), {RoleId,RoleName,0,0,{RoleClass,RoleGender,RoleLevel},Node})),
			put(ranks_info,lists:reverse(lists:keysort(3,get(ranks_info))));
		{RoleId,_,Ranks,Kills,ExtInfo,_}->		%%rejoin? now reset
			put(ranks_info,lists:keyreplace(RoleId,1, get(ranks_info), {RoleId,RoleName,0,0,ExtInfo,Node})),
			put(ranks_info,lists:reverse(lists:keysort(3,get(ranks_info))))
	end,
	{BattleType,BattleId,_} = get(tangle_info),
	TempInfo = lists:filter(fun({_,_,ScoreTmp,_,_,_})-> ScoreTmp =/= -1 end,get(ranks_info)),
	AllInfo = lists:map(fun({RoleIdTmp,RoleNameTmp,ScoreTmp1,KillsTmp,{ClassTmp,GenderTmp,LevelTmp},_})-> battle_ground_packet:make_tangle_battle_role(RoleIdTmp,RoleNameTmp,KillsTmp,ScoreTmp1,GenderTmp,ClassTmp,LevelTmp) end,TempInfo),
	...
	% 荣誉值
	HonorList = get(honor),
	case lists:keyfind(RoleId, 1, HonorList) of
		false ->
			put(honor, HonorList ++ [{RoleId, 0 ,Node}]),
			Msg = battle_ground_packet:encode_battle_end_s2c(0, 0),
			% rpc:call(Node,role_pos_util,send_to_role_clinet,[RoleId, Msg]);
			role_pos_util:send_to_role_clinet(RoleId, Msg);
		{RoleId, Honor ,_} ->
			Msg = battle_ground_packet:encode_battle_end_s2c(Honor, 0),
			% rpc:call(Node,role_pos_util,send_to_role_clinet,[RoleId, Msg])
			role_pos_util:send_to_role_clinet(RoleId, Msg)
	end,
	[ActivityInfo | _] = answer_db:get_activity_info(?TANGLE_BATTLE_ACTIVITY),
	...
	case get(is_battling) of
		true->
			...
			case lists:keyfind(RoleId,1,get(ranks_info)) of
				false->
					nothing;
				{RoleId,RoleName,_Ranks,Kills,ExtInfo,Node}->
					rpc:call(Node,gm_logger_role,insert_log_activity,[RoleId,4,0,0,0,0,JoinTime,LeaveTime]),
					put(ranks_info,lists:keyreplace(RoleId,1, get(ranks_info), {RoleId,RoleName,-1,Kills,ExtInfo,Node})),
					put(ranks_info,lists:reverse(lists:keysort(3,get(ranks_info)))),
				...
				role->
					case creature_op:what_creature(BeKilled) of
						...
						role->
							...
							% 杀人获得荣誉
							get_honor_by_kill_enemy(Killer),
...
% 杀人获得荣誉
get_honor_by_kill_enemy(KillerId) ->
	HonorList = get(honor),
	case lists:keyfind(KillerId, 1, HonorList) of
		false ->
			error;
		{KillerId, Honor ,Node} ->
			case Honor >= ?TANGLE_MAX_HONOR of
				true ->
					ok;
				false ->
					% rpc:call(Node,role_pos_util,send_to_role,[KillerId, {battlefield_reward, ?TANGLE_BATTLE, ?KILL_HONOR}]),
					role_pos_util:send_to_role(KillerId, {battlefield_reward, ?TANGLE_BATTLE, ?KILL_HONOR}),
					put(honor, lists:keyreplace(KillerId, 1, HonorList, {KillerId, Honor + ?KILL_HONOR,Node}))
			end
	end.
%%tangle_kill_info:[killinfo],
%%killinfo:{roleid,{[{bekillroleid,num}],[{killroleid,num}]}}
...
get_role_tangle_kill_info(RoleId)->
	...
	{_,_,_,_,_,Node} = lists:keyfind(RoleId,1,RankInfo),
...
killed_role_broad_cast(Type,MyInfo,OtherInfo,Score)->
    ...
    MyRoleId = creature_op:get_id_from_creature_info(MyInfo),
    KilledRoleId = creature_op:get_id_from_creature_info(OtherInfo),
	case (MyInfo=:=undefined) or (OtherInfo=:=undefined) of
		...
		_ ->
			if Type=:=role->
					...
					travel_battle_util:cast_for_all_server(chat_manager,system_broadcast,[?SYSTEM_CHAT_TANGLE_BATTLE_ROLE_KILLED,MsgInfo,[]]);
				true->
                    nothing
...
notify_role_reward()->
	Fun = fun({RoleId,_,_,_,_,Node})->
					[{Honor,Exp,_},Rank1] = case get_role_score(RoleId) of
										-1->
											[{0,0,[]},0];
										Score->
											Rank = get_my_rank_by_score(Score),
											[get_rewards_by_rank(Rank),Rank]
									end,
					% rpc:call(Node,role_pos_util,send_to_role,[RoleId, {battle_reward_honor_exp,?TANGLE_BATTLE, Honor, Exp}])
					% 记录离开时间
					 leave_time(RoleId),
                     LeaveTime = get_leave_time(RoleId),
                     JoinTime = get_join_time(RoleId),
					rpc:call(Node,gm_logger_role,insert_log_activity,[RoleId,4,0,0,Rank1,Honor,JoinTime,LeaveTime]),
					role_pos_util:send_to_role(RoleId, {battle_reward_honor_exp,?TANGLE_BATTLE, {Honor,Node}, Exp})
			end,
	lists:foreach(Fun, get(ranks_info)).

send_reward_mail()->
	Fun = fun({RoleId,RoleName,_,_,_,Node})->
				  case lists:member(RoleId, get(has_reward)) of
					  false->
						...
						lists:foreach(fun({ItemId,Count})->
											rpc:call(Node,gm_op,gm_send_rpc,[FromName,RoleName,Title,ContextFormat,ItemId,Count,0])
...
send_to_ground(Message)->
	lists:foreach(fun({RoleId,_,_,_,_,Node})->
			case lists:member(RoleId, get(has_leave)) of
				false->
					% rpc:call(Node,role_op,send_to_other_role,[RoleId,Message]);
...
send_to_ground_client(Message)->
	lists:foreach(fun({RoleId,_,_,_,_,Node})->
			case lists:member(RoleId, get(has_leave)) of
				false->
					% rpc:call(Node,role_op,send_to_other_client,[RoleId,Message]);
...
send_to_ground_client_but(Message,NotId)->
	lists:foreach(fun({RoleId,_,_,_,_,Node})->
				...
				case not lists:member(RoleId, get(has_leave)) of
					true-> 
						role_op:send_to_other_client(RoleId,Message);
						% rpc:call(Node,role_op,send_to_other_client,[RoleId, Message]);
					_->
...
%%
notify_manager_battle_start(BattleType,BattleId)->
	travel_tangle_manager:notify_manager_battle_start({BattleType,BattleId}).

notify_manager_role_leave()->
%%	io:format("notify_manager_role_leave ~n"),
	{BattleType,BattleId,_} = get(tangle_info),
	travel_tangle_manager:notify_manager_role_leave({BattleType,BattleId}).
