...
get_camp(Node, MapProcName) ->
	gen_server:call({MapProcName, Node}, get_camp).

get_leave_time(Node, MapProcName, RoleId) ->
	gen_server:call({MapProcName, Node}, {get_leave_time, RoleId}).
...
handle_call(get_camp, _From, State) ->
	Reply = my_apply(get(battle_type), get_camp, []),
	{reply, Reply, State};

handle_call({get_leave_time, RoleId}, _From, State) ->
	Reply = my_apply(get(battle_type), get_leave_time, [RoleId]),
	{reply, Reply, State};
