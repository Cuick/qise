		...
		{_,Memberlist,Mapproc,Besttime,Starttime,Duration}->
			case lists:member(RoleId,Memberlist) of
				false->
					...
					EntryMessage = loop_instance_packet:encode_entry_loop_instance_s2c(Layer,Result,LeftTime_s,Besttime),
	...
	case instanceid_generator:get_procname({GroupId,Type,Layer}) of
		...
		MapProc->
			case map_manager:start_instance(MapProc,MapId,instance,
				{CreatorLevel,?CREATOR_BY_SYSTEM},
				{atom_to_list(ProcName),InstanceProto}) of
				ok->
					%% spawn npc
