...
-export([whereis_role/1,init_role/2,sendmsg/3,send_binary_msg/3,send_world/6,send_map/7,send_map/8,send_instance/7,send_broadcast_world/2,
		send_broadcast_map/3,send_broadcast_instance/3,send_broadcast_map/4,send_broadcast_rolelist/3]).
...
%%          {stop, Reason, State}            (terminate/2 is called)
%% --------------------------------------------------------------------
handle_info({send_msg,{Type,RoleId,RoleName,DestName,Msg,Details,SendList,RoleIden}}, State) ->
	Message = chat_manager:get_filter_msg(Msg),
	case Type of
		?CHAT_TYPE_WORLD->
			send_world(Type,RoleId, RoleName,Message,Details,RoleIden); 
		?CHAT_TYPE_INTHEVIEW->
			send_intheview(Type,RoleId, RoleName,Message,Details,SendList,RoleIden);
		?CHAT_TYPE_PRIVATECHAT->
			send_privatechat(Type, RoleId,RoleName,DestName, Message,Details,SendList,RoleIden);
		?CHAT_TYPE_GROUP->
			send_group(Type,RoleId, RoleName,Message,Details,SendList,RoleIden);
		?CHAT_TYPE_GM_NOTICE->
			send_world(Type,RoleId, RoleName,Msg,Details,RoleIden);
		?CHAT_TYPE_GUILD->
			send_guild(Type,RoleId,RoleName, Message,Details,SendList,RoleIden);
		?CHAT_TYPE_ROLLTEXT->
...
sendmsg(ChatNode,ChatProc,{Type,RoleId,RoleName, DestName,Msg,Details,SendList,RoleIden})->
	gs_rpc:cast(ChatNode, ChatProc, {send_msg,{Type,RoleId,RoleName,DestName,Msg,Details,SendList,RoleIden}}).
...
send_broadcast_rolelist(Id,RoleList,Param)->
    Message = chat_packet:encode_system_broadcast_s2c(Id, Param),
    role_pos_util:send_to_clinet_list(Message, RoleList).

send_direct_to_gate(GateProc,Message)->
	tcp_client:send_data( GateProc, Message).

send_intheview(Type,RoleId,RoleName, Msg,Details,SendList,RoleIden)->
	send_list_msg(Type,RoleId,SendList,RoleName,Msg,Details,RoleIden).	

send_privatechat(Type,RoleId,RoleName, DestName,Msg,Details,[DesId],RoleIden)->
	Messageback = chat_packet:encode_chat_s2c(Type,?SRC_CHAT,DesId,DestName,Msg,Details,RoleIden),
	send_data_to_gate(Messageback),
	send_list_msg(Type,RoleId,[DesId],RoleName,Msg,Details,RoleIden).

send_group(Type,RoleId,RoleName, Msg,Details,SendList,RoleIden)->
	send_list_msg(Type,RoleId,SendList,RoleName,Msg,Details,RoleIden).

send_guild(Type,RoleId,RoleName, Msg,Details,SendList,RoleIden)->
	send_list_msg(Type,RoleId,SendList,RoleName,Msg,Details,RoleIden).
