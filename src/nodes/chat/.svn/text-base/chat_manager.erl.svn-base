...
handle_info({system_broadcast_rolelist,Id,Param,RoleList},State)->
    safe_apply(chat_process,send_broadcast_rolelist,[Id,RoleList,Param]),
    {noreply, State};
...
system_broadcast(Id,Param,{rolelist,RoleList})->
    case get(chat_node_name) of
        undefined->
            case lines_manager:get_chat_name() of
                {Node,_,_}->
    ...
    if
        Node=/= []->
            gs_rpc:cast(Node, ?MODULE, {system_broadcast_rolelist,Id,Param,RoleList});
        true->
            nothing
    end;
