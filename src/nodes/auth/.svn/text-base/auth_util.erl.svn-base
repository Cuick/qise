...
%% Exported Functions
%%
-export([escape_uri/1,binary_to_hexstring/1,platform_check/2,check_time/1]).
...
platform_check(AuthStr, Sign) ->
    MD5Bin = erlang:md5(http_uri:encode(AuthStr)),
    MD5Str = auth_util:binary_to_hexstring(MD5Bin),
    AuthResult = string:to_upper(Sign),
    string:equal(MD5Str, AuthResult).

check_time(Timestamp) ->
    Time = list_to_integer(Timestamp),
    CfgTimeOut=env:get(apiauthttimeout, 3600),
