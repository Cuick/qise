...
handle_info(Info, State) ->
	try
		case Info of
			...
			{regist_mapmanager, {Node, Name, Type}} ->
			...
			{line_load_map, {LineName, MapNodeName}}->
				start_map_processor(LineName, MapNodeName,0);
...
%% @spec load_map() -> void().
load_map(MapNode, Type) ->
	Lines = env:get(lines, []),
	MapConfigFlag = env:get(mapconfig_flag, []),
	lists:foreach(fun(LineId) ->
				      start_map_processor(LineId, MapNode, Type)
		      end, Lines).
	
% start_map_processor(LineId, MapNode)->
% 	CheckLoad = 
% 		case travel_battle_util:is_tavel_battle_map_node(MapNode) of
% 			true->
% 				true;
% 			_->
...
% 					map_manager:start_map_processor(MapNode, LineId, 
% 						MapId, normal, CreatorTag, [])					  
% 				end, AllMaps);
% 		true->
% 			nothing
% 	end.
	...
	if
		CheckLoad->
			...
			AllMaps = if
						Type =:= 1 ->
							% [1205,1206];
							env:get(travel_battle_map1, []);
						true ->
							map_info_db:get_maps_bylinetag(LineId)
					end,
			% AllMaps = map_info_db:get_maps_bylinetag(LineId),
			CreatorTag = {?CREATOR_LEVEL_BY_SYSTEM,?CREATOR_BY_SYSTEM},
			lists:foreach(fun(MapId)->
