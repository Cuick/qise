...
-export([member_online/3,member_levelup/3,create/3,set_leader/3,apply_guild/2,
...
create(Roid,GuildObject,Level)->
	global_util:call(?MODULE,{create_guild, {Roid,GuildObject,Level}}).
...
clear_jszd_battle()->
	global_util:call(?MODULE,{clear_jszd_battle}).
...
handle_call({create_guild, {Roleid,GuildObject,Level}},_From, State) ->
    Reply = my_apply(guild_manager_handle,handle_create,[Roleid,GuildObject,Level]),
    {reply, Reply, State};
...
handle_call({clear_jszd_battle},_From,State)->
	Reply = my_apply(guild_manager_op,clear_jszd_battle,[]),
	{reply, Reply, State};
