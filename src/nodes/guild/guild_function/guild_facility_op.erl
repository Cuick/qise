...
%% opt data
%% ===========================================================
create_guild_to_list(GuildInfo)->
	...
	update_facility_info({{GuildId,?GUILD_FACILITY},GuildLevel,0,0,0,0}),
			...
			if 	
				StartTime =/= 0->			
                  ...
                  % 帮会升级
                            ...
                            % 取id 会取到名字
                            GuildName = guild_spawn_db:get_guild_id(GuildInfo),
                            slogger:msg("guildid info ~p", [GuildInfo]),
                            slogger:msg("guildid name ~p", [GuildName]),
                            slogger:msg("guildid level ~p", [Level+1]),
                            broadcast_op:guild_upgrade(Facid, GuildName, Level+1);
						false->
							nothing
					end;
				true->
...
change_facility_info(NewFainfo)->							
	...
	GuildId,
	FacilityId,
	get_by_facility_item(level,NewFainfo),
