...
init()->
	...
	ets:new(guild_member_binmsg, [set, protected, named_table, {keypos, #update_guild_member_msg.guild_id}]),
...
get_member_info_list(MemberIdList) ->
	get_member_info_list(MemberIdList, []).

get_member_info_list([], MemberInfoList) ->
	MemberInfoList;
get_member_info_list([H|T], MemberInfoList) ->
	case get_member_info(H) of
		[] ->
			get_member_info_list(T, MemberInfoList);
		MemberInfo ->
			get_member_info_list(T, MemberInfoList ++ [MemberInfo])
	end.
...
%	EtsTerm = {{GH,GL,MemberId}},
	case ets:lookup(guild_member_binmsg, GuildId) of
		[] ->
			ets:insert(guild_member_binmsg, #update_guild_member_msg{guild_id = GuildId, role_id_list = [MemberId]});
		[UpdateGuildMemberMsg] ->
			OldRoleIdList = UpdateGuildMemberMsg#update_guild_member_msg.role_id_list,
			case lists:member(MemberId, OldRoleIdList) of
				true ->
					ok;
				false ->
					ets:insert(guild_member_binmsg, UpdateGuildMemberMsg#update_guild_member_msg{role_id_list = OldRoleIdList ++ [MemberId]})
			end
	end.
...
proc_set_leader(GuildId,LeaderId,Roleid)->	 	
	...
	title_op:hook_guild_battle_title(LeaderId,Roleid),
		...
		[]->
			...
			if
				...
				true->
					...
					[G] = guild_packet:make_roleinfo([NewMemberInfo]),
					Msg_Add_Client = guild_packet:encode_guild_member_add_s2c(G), 
