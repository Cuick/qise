...
proc_create(Roleid,Name, Level)->
	case guild_spawn_db:create_guild(Name,Level) of
		 {failed,invalid_name}->
		 		GuildObject = [],
	...
	if
		...
		true->
			...
			CreateSilver = guild_spawn_db:get_guild_silver(GuildObject),
			...
			Treasure_transport_start_Time = guild_spawn_db:get_guild_treasure_transport(GuildObject),
			%%创建组员
			guild_spawn_db:add_member_to_guild(CreateId,Roleid,?GUILD_POSE_LEADER),
			%%创建设施
			guild_spawn_db:set_guild_facility(CreateId,?GUILD_FACILITY,Level,0,0,0,0),
            ...
            MessageGuild2 = {guildcreate_msg, {guildname, Name}},
            role_pos_util:send_to_role(Roleid, MessageGuild2),
...
proc_get_recruite_info(RoleId)->
	...
	AllGuildRecuiInfo = ets:foldl(fun(GuildInfo,Acc)->
			...
			try
				...
				Old_TopGuild = get_top_guild1(1,LeaderAndFormalFun),
				...
				New_TopGuild = get_top_guild1(1,LeaderAndFormalFun),	
				% update_role_title(Old_TopGuild,New_TopGuild),
				[guild_packet:make_recruiteinfo(GuildId ,GuildName ,
				GuildLevel, Membernum,FormalNum,LeaderName, Restrict, Facslevel, ApplyFlag,CreateDate,SortIndex)] ++ Acc
			catch
...
update_role_title(Old_TopGuild,New_TopGuild) ->
	New_TopGuild_Length = length(New_TopGuild),
	if
		New_TopGuild_Length =:= 0 ->
			nothing;
		true->
			Old_TopGuild_Length = length(Old_TopGuild),
			if
				Old_TopGuild_Length =:= 0 ->
					[{GuildId,NewLeader,Rank}]= New_TopGuild,
					do_update_role_title(NewLeader,1);
				true->
					[{GuildId,OldLeader,Rank}]= Old_TopGuild,
					[{GuildId,NewLeader,Rank}]= New_TopGuild,
					do_update_role_title(NewLeader,1),
					do_update_role_title(OldLeader,-1)
			end
	end.
do_update_role_title(RoleId,Rank)->
	case role_pos_util:is_role_online(RoleId) of
		...
		false ->
			title_op:hook_on_condition_change_offline(
				?TITLE_TYPE_GUILD, RoleId, 1)
	end.
get_top_guild1(Top,LeaderAndFormalFun)->
	...
	lists:foldl(fun({GuildId,Rank},Acc)->
					GuildInfo = get_guild_info(GuildId),
					Members = get_by_guild_item(members,GuildInfo),	
					{Leader,_} = lists:foldl(LeaderAndFormalFun,{0,0},Members),
					% GuildName = get_by_guild_item(name,GuildInfo),
					Acc ++ [{GuildId,Leader,Rank}]
				end, [], Ranks),
	RanksInfo.
...
proc_guild_disband(RoleId,GuildId)->
	...
	case (GuildInfo=/=[]) of
		true->
			...
			title_op:hook_guild_battle_title(RoleId,0),
			%%gm log
...
send_guild_info_to_client(Roleid,GuildId)->
	...
	SendMembers = lists:map(fun(Memberid)->
						MemberInfo = guild_member_op:get_member_info(Memberid),
						[G] = guild_packet:make_roleinfo([MemberInfo]),
						G				
						end ,MemberLists),
...
%				nothing;
%		UpdateGuildMemberMsg ->
%				{GH,GL,MemberId} = FirstKey,
				%%io:format("get_member_info MemberId ~p ~n",[MemberId]),
	case ets:tab2list(guild_member_binmsg) of
		[] ->
			ok;
		UpdateGuildMemberMsgList ->
%				#update_guild_member_msg{guild_id = GuildId, role_id_list = RoleIdList} = UpdateGuildMemberMsg,
%				case guild_member_op:get_member_info_list(RoleIdList) of
%					[]->
%						%%io:format("get_member_info [] ~n"),
%						nothing;
%					MemberInfoList ->
%						MessageMember = guild_packet:encode_guild_member_update_s2c(guild_packet:make_roleinfo(MemberInfoList)),
%						broad_cast_to_guild_client(GuildId, MessageMember)
%				end,
			sent_memberbinmsg_to_client_step(UpdateGuildMemberMsgList)
	end,
	ets:delete_all_objects(guild_member_binmsg).

sent_memberbinmsg_to_client_step([]) ->
	ok;
sent_memberbinmsg_to_client_step([H|T]) ->
	...
	#update_guild_member_msg{guild_id = GuildId, role_id_list = RoleIdList} = H,
	case guild_member_op:get_member_info_list(RoleIdList) of
		[]->
			nothing;
		MemberInfoList ->
	...
	sent_memberbinmsg_to_client_step(T).
%	end.	
