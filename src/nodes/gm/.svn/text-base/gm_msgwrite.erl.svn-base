...
%% --------------------------------------------------------------------
%% External exports
-export([start_link/0,write/2,write2/3,write3/3,gm_log_control/2]).
...
write3(Table, KeyValueList, Type) ->
	case get(gm_node) of
		undefined->
			case node_util:get_gmnode() of
				undefined-> slogger:msg("gm_msgwrite error : can not find gm node~n");
				GmNode->
					put(gm_node,GmNode),
					gs_rpc:cast(GmNode, ?MODULE, {msg_write3,KeyValueList,Table,Type})
			end;
		GmNode->
			gs_rpc:cast(GmNode, ?MODULE, {msg_write3,KeyValueList,Table,Type})
	end.
...
handle_info({msg_write3,KeyValueList,Table,Type},State) ->
	abstract_write_db3(Table,KeyValueList,Type),
	{noreply, State};
...
abstract_write_db3(_,KeyValueList,Type) ->
	case Type of
		buffer->
			gm_msgwrite_mysql:write_db_to_buffer3(KeyValueList);
		directly->
			gm_msgwrite_mysql:write_db3(KeyValueList);
		merge->
			gm_msgwrite_mysql:write_db_to_merge3(KeyValueList);
		nodb->
			nothing;
		_->
			nothing
