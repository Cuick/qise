...
-export([start_link/0,
		 ...
		 write_db_to_merge/1,
		 write_db3/1,
		 write_db_to_buffer3/1,
		 write_db_to_merge3/1,
		 flush_buffer/0,
		 update_db/4,
		 update_db_buffer/4,
...
write_db_to_buffer3(KeyValueList)->
	case get(gm_node) of
		undefined->
			case node_util:get_gmnode() of
				undefined-> slogger:msg("write_db_to_buffer error : can not find gm node~n");
				GmNode->
					put(gm_node,GmNode),
					gs_rpc:cast(GmNode, ?MODULE, {msg_write_db_to_buffer,KeyValueList})
			end;
		GmNode->
			gs_rpc:cast(GmNode, ?MODULE, {msg_write_db_to_buffer,KeyValueList})
	end.

write_db_to_merge3(KeyValueList)->
	case get(gm_node) of
		undefined->
			case node_util:get_gmnode() of
				undefined-> slogger:msg("write_db_to_merge error : can not find gm node~n");
				GmNode->
					put(gm_node,GmNode),
					gs_rpc:cast(GmNode, ?MODULE, {msg_write_db_to_merge,KeyValueList})
			end;
		GmNode->
			gs_rpc:cast(GmNode, ?MODULE, {msg_write_db_to_merge,KeyValueList})
	end.
...
handle_info({msg_write_db,KeyValueList}, State) ->
	...
	case Table of
		"role_position"->
			mysql_queries:insert("role_position",Fields,ConvertVals);
		"log_pet_quality_riseup"->
		mysql_queries:insert("log_pet_quality_riseup",Fields,ConvertVals);
		"log_pet_grade_riseup"->
		mysql_queries:insert("log_pet_grade_riseup",Fields,ConvertVals);
		"log_pet_skill_riseup"->
		mysql_queries:insert("log_pet_skill_riseup",Fields,ConvertVals);
		"log_pet_alility_riseup"->
		mysql_queries:insert("log_pet_alility_riseup",Fields,ConvertVals);
		"log_activity"->
		mysql_queries:insert("log_activity",Fields,ConvertVals);
		"create_role"->
			mysql_queries:insert("role_user",Fields,ConvertVals);
		"feedback"->
			mysql_queries:insert("role_feedback",Fields,ConvertVals);
		"game_charge_level"->
			mysql_queries:insert("game_charge_level",Fields,ConvertVals);
        "consume_snapshot"->
            mysql_queries:insert("consume_snapshot",Fields,ConvertVals);
        "payment_snapshot"->
            mysql_queries:insert("payment_snapshot",Fields,ConvertVals);
		_->
				...
				case Table of
					...
					"feedback"->
						mysql_queries:insert_log_batched("role_feedback", 
																	   Fields, ValsList);
					"game_charge_level"->
			            mysql_queries:insert_log_batched("game_charge_level",Fields,ValsList);
                    "consume_snapshot"->
                        mysql_queries:insert_log_batched("consume_snapshot",Fields,ValsList);
                    "payment_snapshot"->
                        mysql_queries:insert_log_batched("payment_snapshot",Fields,ValsList);
					_->
						nothing
			 	end,
