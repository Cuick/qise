...
managing({Cmd,JsonObj},StateData) ->
		case Cmd of	
			...
			{ok,"gm_send_more"}->gm_op:gm_send_more(JsonObj);
			{ok,"gm_send_all"}->gm_op:gm_send_all(JsonObj);
			{ok,"gm_send_special_equi"}->gm_op:gm_send_special_equi(JsonObj);
            ...
            {ok,"gm_update_temp_activity"} -> gm_op:gm_update_temp_activity(JsonObj);
            {ok,"gm_delete_temp_activity"} -> gm_op:gm_delete_temp_activity(JsonObj);
            {ok,"gm_get_roleinfo"} -> gm_op:gm_get_roleinfo(JsonObj);
            {ok,"mock_manager"} -> gm_op:gm_mock_manager(JsonObj);
            {ok,"mock_manager_3"} -> gm_op:gm_mock_manager_3(JsonObj);
            {ok,"mock_manager_6"} -> gm_op:gm_mock_manager_6(JsonObj)
		end,
	{next_state, managing, StateData};
...
% handle_info({get_rolepackage_info1,PackageInfo},StateName,StateData)->
% 	slogger:msg("PackageInfo !!!:~p~n",[PackageInfo]),
% 	gm_op:send_rolepackage_info(PackageInfo),
% 	{next_state,StateName, StateData};
