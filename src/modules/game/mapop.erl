...
get_map_can_sitdown(MapId)->
	case map_info_db:get_map_info(MapId) of
		[]->
			0;
		MapInfo->
			map_info_db:get_can_sitdown(MapInfo)
	end.

get_map_can_ride(MapId)->
	case map_info_db:get_map_info(MapId) of
		[]->
			0;
		MapInfo->
			map_info_db:get_can_ride(MapInfo)
	end.
...
get_map_pos_tag(Coord, MapDb)->
	    ...
	    mapdb_processor:query_map_stand(MapDb, Coord)
            ...
            error:badarg ->
                slogger:msg("get_map_pos_tag: ~p,~p~n",[MapDb,Coord]),
