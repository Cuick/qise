...
get_server_start_time()->
	...
	StartTimeList = env:get(server_start_time,[]),
	case lists:keyfind(ServerId, 1, StartTimeList) of
		false->
...
get_server_combine_time()->
	ServerId = env:get(serverid,0),
	StartTimeList = env:get(server_combine_time,[]),
	case lists:keyfind(ServerId, 1, StartTimeList) of
		false->
			slogger:msg("not find server_combine_time~n"),
			{{1970,1,1},{0,0,0}};
		{_,ServerCombineTime}->
			ServerCombineTime
	end.
	% env:get(server_combine_time,[]).
...
is_success_by_probability(Probability)->
    ...
    if 
		Random_num < Probability -> true;
		true -> false
	end.

%% �������б�ת�����ַ��б?�磺[1,2]->["1","2"]
int_list_to_string_list(Int_list)->
	[integer_to_list(trunc(Number)) || Number <- Int_list]. 

%% ��ԭ�Ӳ���ֵת�������β���ֵ���磺true->1
atom_bool_to_int_bool(Atom_bool)->
	case Atom_bool of
		true-> 1;
		false->0
	end.
...
% 从1-10000取一个随即数
random_num() ->
	{N,_Time} = random:uniform_s(10000,now()),
	N.

% 计算概率(以10000为基数)
is_success(SuccessRate) ->
	random_num() =< SuccessRate.

% 当前时间(秒)
now_sec() ->
	{Mega, Secs, Micro} = erlang:now(),
	Mega * 1000000 + Secs + Micro div 1000000.

% 从一个List里随机Counter个元素,组成新List
do_random(Counter, List) ->
	do_random(Counter, List, []).

do_random(0, _List, LastList) ->
	LastList;
do_random(Counter, List, LastList) ->
	NewElement = lists:nth(random:uniform(length(List)), List),
	do_random(Counter - 1, List, LastList ++ [NewElement]).

% 从系统通用ets表中导出List并去掉表中键值
tab2list(EtsName) ->
	case ets:tab2list(EtsName) of
		[] ->
			[];
		EtsList ->
			deletekey(EtsList, [])
...
deletekey([], LastList) ->
	LastList;
deletekey([H|T], LastList) ->
	{_Key, Value} = H,
	deletekey(T, LastList ++ [Value]).
...
% 当前时间是否在某时间内
is_time({StartYear, StartMonth, StartDay, StartHour, StartMinute, StartSecond}, {EndYear, EndMonth, EndDay, EndHour, EndMinute, EndSecond}) ->
	{{NowYear, NowMonth, NowDay}, {NowHour, NowMinute, NowSecond}} = erlang:localtime(),
	StartTime = list_to_integer(integer_to_list(StartYear) ++ bu_ling(StartMonth) ++ bu_ling(StartDay) ++ bu_ling(StartHour) ++ bu_ling(StartMinute) ++ bu_ling(StartSecond)),
	EndTime = list_to_integer(integer_to_list(EndYear) ++ bu_ling(EndMonth) ++ bu_ling(EndDay) ++ bu_ling(EndHour) ++ bu_ling(EndMinute) ++ bu_ling(EndSecond)),
	NowTime = list_to_integer(integer_to_list(NowYear) ++ bu_ling(NowMonth) ++ bu_ling(NowDay) ++ bu_ling(NowHour) ++ bu_ling(NowMinute) ++ bu_ling(NowSecond)),
...
% 某时间是否在某断时间内
is_time({StartYear, StartMonth, StartDay, StartHour, StartMinute, StartSecond}, {{Year, Month, Day}, {Hour, Minute, Second}}, {EndYear, EndMonth, EndDay, EndHour, EndMinute, EndSecond}) ->
	StartTime = list_to_integer(integer_to_list(StartYear) ++ bu_ling(StartMonth) ++ bu_ling(StartDay) ++ bu_ling(StartHour) ++ bu_ling(StartMinute) ++ bu_ling(StartSecond)),
	EndTime = list_to_integer(integer_to_list(EndYear) ++ bu_ling(EndMonth) ++ bu_ling(EndDay) ++ bu_ling(EndHour) ++ bu_ling(EndMinute) ++ bu_ling(EndSecond)),
	Time = list_to_integer(integer_to_list(Year) ++ bu_ling(Month) ++ bu_ling(Day) ++ bu_ling(Hour) ++ bu_ling(Minute) ++ bu_ling(Second)),
	Time >= StartTime andalso Time =< EndTime.

% 是否换日
is_change_day({{Year, Month, Day}, {_Hour, _Minute, _Second}}) ->
	{{NowYear, NowMonth, NowDay}, {_NowHour, _NowMinute, _NowSecond}} = erlang:localtime(),
	NowTime = list_to_integer(integer_to_list(NowYear) ++ bu_ling(NowMonth) ++ bu_ling(NowDay)),
	Time = list_to_integer(integer_to_list(Year) ++ bu_ling(Month) ++ bu_ling(Day)),
	Time =/= NowTime.

random_choice(ItemList) ->
	SumRate = lists:foldl(fun({_, RateTmp}, Acc) ->
		Acc + RateTmp
	end, 0, ItemList),
	Rate = random:uniform(SumRate),
	{ItemId, _} = lists:foldl(fun({ItemIdTmp, RateTmp}, {ItemId2, Rate2}) ->
		if
			ItemId2 =:= [] ->
				CurRate = Rate2 - RateTmp,
				if
					CurRate =<0 -> 
						{ItemIdTmp,0};
					true->
