...
-export([get_online_roleid_by_name/1,get_role_pos/1]).
	...
	case role_server_travel:is_in_travel() of
		...
		false->	
			case role_server_travel:is_same_source_role(RoleId) of
				...
				_->
					case travel_battle_deamon_op:get_one_connect_map_node() of
						[]->
							[];
						ShareNode->		
...
where_is_role(RoleName) ->
	case role_pos_db:get_role_pos_from_mnesia_by_name(RoleName) of
		[] ->
			case role_server_travel:is_in_travel() of
				true ->
...
where_is_role_travel_channel(ServerId,RoleIdOrName)->
	case where_is_role(RoleIdOrName) of
		[]->			%% not in share_map
			case travel_battle_map_travel_op:get_source_node_by_serverid(ServerId) of
				[]->
					[];
...
get_role_pos(RoleId) ->
	case where_is_role(RoleId) of
		[]->
			undefined;
		RolePos->
			{role_pos_db:get_role_pid(RolePos), role_pos_db:get_role_mapnode(RolePos)}
	end.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%								send to role !!!						%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
where_is_role_by_serverid(ServerId,RoleNameOrId)->
	slogger:msg("where_is_role_by_serverid ServerId ~p ~n",[ServerId]),
