...
make_role_param(RoleId, RoleName, ServerId) ->
	Name = util:safe_binary_to_list(RoleName),
	chat_packet:makeparam(role,{Name, RoleId,ServerId}).
...
make_title_param(Title) ->
	chat_packet:makeparam(title,Title).
