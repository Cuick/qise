...
effect(_Value,_SkillInput)->
	...
	case get_state_from_roleinfo(get(creature_info)) of 
		sitting ->
			...
			AddExp = trunc(role_level_sitdown_effect_db:get_exp(EffectInfo)*(1+CompanoinAddExt+GlobalRate)),
			AddVipExp = trunc(role_level_sitdown_effect_db:get_exp(EffectInfo)*VipAdd),
			...
			AddSoulPower = trunc(role_level_sitdown_effect_db:get_soulpower(EffectInfo)*(1+CompanoinAddExt)),
			AddVipSoulPower = trunc(role_level_sitdown_effect_db:get_soulpower(EffectInfo)*VipAdd),
			...
			SelfEffUp = MpEffUp ++ HpEffUp ++ [{soulpower,AddSoulPower + AddVipSoulPower},{expr,AddExp + AddVipExp}],
			%%1.set value
...
%%			role_op:broadcast_message_to_aoi_client(Message),  not send to aoi
            ...
            role_op:obtain_exp([{expr, AddExp}, {vip_expr, AddVipExp}]),
            role_op:obtain_soulpower([{soulpower, AddSoulPower}, {vip_soulpower, AddVipSoulPower}]),
			[];	
		_->
			remove
	end.
