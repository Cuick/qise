...
effect(_Value,_SkillInput)->
	...
	case role_level_bonefire_db:get_bonefire_exp_info(CurLevel) of
		...
		Info->
			...
			AddExp = trunc(Exp * ExpRate),
			AddSoulPower = trunc(SoulPower * ExpRate),
			AddVipExp = trunc(Exp * VipAdd),
			AddVipSoulPower = trunc(SoulPower * VipAdd),
			SelfEffUp = [{soulpower,AddExp + AddVipExp},{expr,AddSoulPower + AddVipSoulPower}],
