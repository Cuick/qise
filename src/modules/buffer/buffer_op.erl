...
put_timer(BufferId,TimerId)->
	case get_timer(BufferId) of
		undefined-> o;
		OldTimerId -> erlang:cancel_timer(OldTimerId)
	end,
	...
	{_,EffectTules} = 
		lists:foldl(fun(Effect,{N,LastEffects})-> 
			case effect:is_attr_effect(Effect) of
				true->
					% slogger:msg("2 ~p~n",[{Effect,EffectArguments,LastEffects}]),
					EffectTuple = {Effect,lists:nth(N, EffectArguments)},
					{N+1,LastEffects++[EffectTuple]};
				_->
					% slogger:msg("1 ~p~n",[{Effect,EffectArguments,LastEffects}]),
					{N+1,LastEffects}
			end end,{1,[]}, Effects),
	EffectTules.	
...
% 离开地图清除某些buffer
clear_buffer_for_level_map(CurrentMapId) ->
	CurrentMapInfo = map_info_db:get_map_info(CurrentMapId),
	ClearBufferList = CurrentMapInfo#map_info.clear_buffer_list,
	case is_list(ClearBufferList) andalso ClearBufferList =/= [] of
		true ->
			% 清除buffer
			case do_clear_buffer(ClearBufferList, []) of
				[] ->
					ok;
				RemoveBufferList ->
					% 重新计算玩家清除掉buffer后的属性
					role_op:recompute_attr([], RemoveBufferList)
			end;
		false ->
			ok
	end.

do_clear_buffer([], RemoveBufferList) ->
	RemoveBufferList;
do_clear_buffer([H|T], RemoveBufferList) ->
	case lists:keyfind(H, 1, get(current_buffer)) of
		false ->
			do_clear_buffer(T, RemoveBufferList);
		{H, BufferLevel} ->
			role_op:remove_without_compute({H, BufferLevel}),
			do_clear_buffer(T, RemoveBufferList ++ [{H, BufferLevel}])
	end.
