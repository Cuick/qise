...
make_ki(KillInfo,AllRoleInfo)->
	lists:foldr(fun({RoleId,Times},TmpInfo)-> 
						case lists:keyfind(RoleId,1,AllRoleInfo) of 
							false->
								TmpInfo;
							{RoleId,RoleName,_Ranks,_Kills,{RoleClass,_RoleGender,RoleLevel},Node}->
...
make_gbinfo_param(GbInfo)->
	lists:map(fun({_,Index,Name,Score,TScore,_})->
					  ...
					  #gbt{index=Index,name=Name,yhzqscore=YhzqScore,jszdscore=JszdScore,score=TScore} 
				end,GbInfo).
...
encode_notify_to_join_yhzq_s2c(Camp)->
	login_pb:encode_notify_to_join_yhzq_s2c(#notify_to_join_yhzq_s2c{camp=Camp}).
...
encode_battle_totleadd_honor_s2c(Honor)->
	login_pb:encode_battle_totleadd_honor_s2c(#battle_totleadd_honor_s2c{num = Honor}).
	
encode_tangle_battle_end_s2c()->
	login_pb:encode_tangle_battle_end_s2c(#tangle_battle_end_s2c{}).
