...
sysbrd({title,TitleId},Count)->
	case equipment_sysbrd_db:get_info({title,TitleId}) of
		[]->
			nothing;
		BrdInfo->
			BrdId = equipment_sysbrd_db:get_brdid(BrdInfo),
			ParamRole = system_chat_util:make_role_param(get(creature_info)),
			ParamTitle = system_chat_util:make_title_param(TitleId),
			MsgInfo = [ParamRole,ParamTitle],
			system_chat_op:system_broadcast(BrdId,MsgInfo)
	end;
sysbrd({smashed_egg,ItemId},Count)->
	case equipment_sysbrd_db:get_info({smashed_egg,ItemId}) of
		[]->
			nothing;
		BrdInfo->
			BrdId = equipment_sysbrd_db:get_brdid(BrdInfo),
			ParamRole = system_chat_util:make_role_param(get(creature_info)),
			ParamItem = system_chat_util:make_item_param(ItemId),
			MsgInfo = [ParamRole,ParamItem],
			system_chat_op:system_broadcast(BrdId,MsgInfo)
	end;
sysbrd({god_tree,ItemId},Count)->
	case equipment_sysbrd_db:get_info({god_tree,ItemId}) of
		[]->
			nothing;
		BrdInfo->
			BrdId = equipment_sysbrd_db:get_brdid(BrdInfo),
			...
			system_chat_op:system_broadcast(BrdId,MsgInfo),
			RoleName = util:safe_binary_to_list(get_name_from_roleinfo(get(creature_info))),
			Msg = god_tree_packet:encode_god_tree_broad_s2c(RoleName,ItemId,Count),
			chat_op:send_binary_message(Msg)
	end;
