...
proc_chat_msg({Type, RoleName, Msg,Details,ServerId})->	
			...
			case parse_cmd(Msg) of
				chat ->
					case is_chat_cooltime_ok(Type,get(chat_info)) of
						true-> 
							check_and_send(Type, RoleName,ServerId, Msg,Details);				
						CdTime->
...
check_and_send(Type, RoleName,ServerId, Msg,Details)->	
	...
	case check_chat_restict(ConditionInfo) of
		true->
			%%发送消息				
			case dispatch_chat_msg(Type, RoleName,ServerId, Msg,Details) of
				true->
...
%%处理消息发送
dispatch_chat_msg(Type, RoleName,ServerId, Msg,Details)->
	case Type of
		?CHAT_TYPE_WORLD->
		...
		?CHAT_TYPE_PRIVATECHAT->
			send_privatechat(Type, RoleName,ServerId,Msg,Details);
		?CHAT_TYPE_GROUP->
...
send_by_loudspeaker(RoleId,RoleName,Msg,Details)->
	...
	travel_battle_util:cast_for_all_server(loudspeaker_manager,send_loudspeaker,[RoleId,{RoleName,Msg,Details,get_role_iden(),ServerId}]),
	true.

%% 向世界发送信息
send_world(Type, RoleName, Msg,Details)->	
	send_by_chat_proc(Type,RoleName,Msg,Details,[]),
	true.
...
send_intheview(Type, RoleName,Msg,Details)->
	...
	case role_server_travel:is_in_travel() of
		true->
			...
			Message = chat_packet:encode_chat_s2c(Type,?DEST_CHAT,get(roleid),RoleName,FilterMsg,Details,get_role_iden(),ServerId),
			role_pos_util:send_to_clinet_list(Message, [get(roleid)|RoleIdList]);
		_->			
			send_by_chat_proc(Type,RoleName,Msg,Details,[get(roleid)]++RoleIdList)
	end,
	true.
		...
		true->
			case role_pos_util:get_online_roleid_by_name(RoleName) of
				[]->
					{error,norole};
				Id->
					send_by_chat_proc(Type,RoleName,Msg,Details,[Id]),
					true
			end;
		_->		%%跨服聊天
			case role_pos_util:where_is_role_by_serverid(ServerId,RoleName) of
				[]->		%%未在线
					{error,norole};
				RolePos->
					Messageback = chat_packet:encode_chat_s2c(Type,?SRC_CHAT,role_pos_db:get_role_id(RolePos),RoleName,Msg,Details,get_role_iden(),ServerId),
					role_op:send_data_to_gate(Messageback),
					Message = chat_packet:encode_chat_s2c(Type,?DEST_CHAT,get(roleid),make_role_name(),Msg,Details,get_role_iden(),MyserverId),
...
%% 获得group信息
	...
	case Member of
		[]->
			{error,nogroup};
		_->	
			send_by_chat_proc(Type,RoleName,Msg,Details,Member),			
			true
	end.		
...
%% 获得guild信息
	...
	case MemberidList of
		[]->
			{error,noguild};
		_->
			send_by_chat_proc(Type,RoleName,Msg,Details,MemberidList),			
			true
	end.
...
send_by_chat_proc(Type,DestName,Msg,Details,SendList)->
	...
	chat_process:sendmsg(ChatNode,ChatProc,{Type,get(roleid),make_role_name(),DestName,Msg,Details,SendList,get_role_iden()}).
					...
					".zymoney "++MoneyStr->
						case check_gmaccount() of
							true->
								...
								role_op:money_change(?MONEY_BOUND_SILVER,Money,got_quest);					 				
							_->
								throw(notgmaccount)
						end;
					".zygold "++GoldStr->
						case check_gmaccount() of
							true->
								...
								role_op:money_change(?MONEY_GOLD,Gold,got_quest);					 				
							_->
								throw(notgmaccount)
						end;	
					".zybt " ++ Block->
...
gm_ban_chat(RoleName) ->
	% 获取roleattr所有数据库文件
	RoleAttrList = dal:get_all_table(roleattr),
	Pattern = #roleattr{name = RoleName, _ = '_'},
	case dal:get_result(Pattern, RoleAttrList) of
		false ->
			ok;
		[RoleAttr] ->
			#roleattr{roleid = RoleId, account = Account} = RoleAttr,
			% 写入禁言数据库
			DurationTime = 10 * 365 * 24 * 60 * 60,
			gm_block_db:add_user(Account, talk, DurationTime),
			% 角色是否在线
			case role_processor:whereis_role(RoleId) of
				undefined ->
					ok;
				RoleProc ->
					% 在线,更新chat_info
					RoleProc ! {update_chat_info, DurationTime}
			end
	end.
