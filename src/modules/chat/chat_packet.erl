...
handle(#chat_c2s{type = Type, desserverid = ServerId,desrolename = Desrolename, msginfo = Msg,details=Details}, RolePid) ->
	role_processor:chat_message(RolePid, {Type, Desrolename, Msg,Details,ServerId});
...
encode_chat_s2c(Type,PrivateFlag,RoleId,RoleName,Msg,Details,RoleIden)->
	encode_chat_s2c(Type,PrivateFlag,RoleId,RoleName,Msg,Details,RoleIden,0).
	
encode_chat_s2c(Type,PrivateFlag,RoleId,RoleName,Msg,Details,RoleIden,Serverid) ->
	login_pb:encode_chat_s2c(#chat_s2c{type = Type, privateflag =PrivateFlag,serverid = Serverid, desroleid=RoleId, desrolename = RoleName, msginfo=Msg,details=Details,identity = RoleIden}).
...
makeparam(title,Title)->
	TempidKV = pb_util:key_value(?TITLE, Title),
	#rkv{kv = [TempidKV],kv_plus=[],color=0};
