...
%% --------------------------------------------------------------------
init([]) ->
	mock_processor_sup_type_6:start_link(),
    {ok, #state{}}.
...
handle_cast({stop_all_mocks}, State) ->
	...
	% 	MockProc = mock_processor_type_6:make_proc_name(MockId),
	% 	mock_processor_type_6:stop(MockProc),
	% 	mock_processor_sup_type_6:stop_child(MockProc)
	% end, get(mock_list)),
	% erase(mock_list),
	todo,
	{noreply, State};
...
do_create_all_mocks() ->
	...
	MockList = lists:foldl(fun(MockInfo, Acc) ->
		ProtoId = mock_db:get_mock_id_type_6(MockInfo),
		Class = mock_db:get_mock_class_type_6(MockInfo),
		Gender = mock_util:get_mock_gender(),
		MockId = mock_id_generator:gen_newid(),
		Name = mock_name_generator:gen_new_name(Gender),
		MockProc = mock_processor_type_6:make_proc_name(MockId),
		mock_processor_sup_type_6:start_mock_type_6(MockProc, MockId, ProtoId, Name, Gender, Class, LineId),
		[MockId | Acc]
	end, [], mock_db:get_all_mock_type_6()),
