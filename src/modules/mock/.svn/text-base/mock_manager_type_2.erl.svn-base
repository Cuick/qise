...
%% --------------------------------------------------------------------
init([]) ->
	mock_processor_sup_type_2:start_link(),
    {ok, #state{}}.
...
handle_cast({stop_all_mocks}, State) ->
	lists:foreach(fun(MockId) ->
		MockProc = mock_processor_type_2:make_proc_name(MockId),
		mock_processor_type_2:stop(MockProc),
		mock_processor_sup_type_2:stop_child(MockProc)
	end, get(mock_list)),
	erase(mock_list),
	{noreply, State};
...
do_create_all_mocks() ->
	...
	MockList = lists:foldl(fun(MockInfo, Acc) ->
		ProtoId = mock_db:get_mock_id_type_2(MockInfo),
		Class = mock_db:get_mock_class_type_2(MockInfo),
		...
		MockProc = mock_processor_type_2:make_proc_name(MockId),
		mock_processor_sup_type_2:start_mock_type_2(MockProc, MockId, ProtoId, Name, Gender, Class, LineId),
		[MockId | Acc]
	end, [], mock_db:get_all_mock_type_2()),
