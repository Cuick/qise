...
get_faction_from_creature_info(CreatureInfo) when is_record(CreatureInfo, gm_role_info) ->
	get_camp_from_roleinfo(CreatureInfo);
...
set_state_to_creature_info(CreatureInfo,Value) when is_record(CreatureInfo, gm_role_info) ->
	set_state_to_roleinfo(CreatureInfo,Value);
set_state_to_creature_info(CreatureInfo,Value) ->
	undefined.
...
get_power_from_creature_info(CreatureInfo) when is_record(CreatureInfo, gm_pet_info) ->
	get_power_from_petinfo(CreatureInfo);
...
set_power_to_creature_info(CreatureInfo,Value) when is_record(CreatureInfo, gm_pet_info) ->
	set_power_to_petinfo(CreatureInfo,Value);
...
get_fighting_force_from_creature_info(CreatureInfo) when is_record(CreatureInfo, gm_role_info) ->
	get_fighting_force_from_roleinfo(CreatureInfo);
get_fighting_force_from_creature_info(CreatureInfo) ->
	undefined.	
	
set_fighting_force_to_creature_info(CreatureInfo,Value) when is_record(CreatureInfo, gm_role_info) ->
	set_fighting_force_to_roleinfo(CreatureInfo,Value);
set_fighting_force_to_creature_info(CreatureInfo,Value) ->
	undefined.

get_gender_from_creature_info(CreatureInfo) when is_record(CreatureInfo, gm_role_info) ->
	get_gender_from_roleinfo(CreatureInfo);
get_gender_from_creature_info(CreatureInfo) ->
	undefined.
...
%% 移动
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pos_update(RoleInfo, MapInfo, Pos,Grid) ->
	MapProcName = get_proc_from_mapinfo(MapInfo),
...
%% 切换位置
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
shift_pos(CreatureInfo, MapInfo, Pos) ->
	SelfId = creature_op:get_id_from_creature_info(CreatureInfo),	
	SelfType = what_creature(SelfId),
	OldPos = creature_op:get_pos_from_creature_info(CreatureInfo),									
	%% 设置新位置
	NewCreatureInfo = set_pos_to_creature_info(CreatureInfo,Pos),
	put(creature_info,NewCreatureInfo),
	update_creature_info(NewCreatureInfo),
	MapProcName = get_proc_from_mapinfo(MapInfo),
	OldGrid = mapop:convert_to_grid_index(OldPos, ?GRID_WIDTH),					
	NewGrid = mapop:convert_to_grid_index(Pos, ?GRID_WIDTH),							
	pos_update(NewCreatureInfo, MapInfo, Pos,NewGrid),
	if
		NewGrid =/= OldGrid->							
			%%% 切换不同的网格
			mapop:leave_grid(SelfId, MapProcName, OldGrid),
			mapop:join_grid(SelfId, NewGrid, MapProcName),
			if
				SelfType =:= role ->		%%玩家更新区域
					mapop:update_grids_state(MapProcName, NewGrid,OldGrid);
				true->		
					case mapop:is_grid_active(MapProcName,NewGrid) of
						false->		%%Npc走到了未激活的区域
							self() ! {hibernate};
						true->
							nothing
...
move_heartbeat(OriRoleInfo, MapInfo, Coord) ->
	...
	case util:is_in_range(LastUpdatePos,Pos,?MOVE_UPDATE_RANGE) of
		false->							
			pos_update(NewCreatureInfo, MapInfo, Pos,New_grid),
...
is_has_relation(SelfInfo,OtherInfo)->
	...
	if
		(is_record(SelfInfo, gm_role_info)) or (is_record(OtherInfo, gm_role_info))->
			true;
		true->
