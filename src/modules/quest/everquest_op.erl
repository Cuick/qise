...
can_accept_everquest(EverProtoInfo,EverInfo)->
	if
		...
		true->
			...
			Last_evquests = everquest_db:get_last_evquest(EverProtoInfo),
			EverquestCheck =
			case Last_evquests of
				[] ->
					true;
				_->
					% QuestList = everquest_db:get_quests(everquest_db:get_info(Last_evquest)),
					QuestList=lists:foldl(fun(X, Sum) -> everquest_db:get_quests(everquest_db:get_info(X)) ++ Sum end, [], Last_evquests),
					case lists:filter(fun(QuesId)->quest_op:has_quest(QuesId) end , QuestList) of
					 	[] ->
					 		true;
					 	_ ->
					 		false
			...
			if 
				(ClassCheck and LevelCheck and QuestCheck and GuildCheck and DateLineCheck and EverquestCheck)->
...
hookon_get_rewards(EverQId,QuestId)->
	...
	case get_cur_everquest_info(EverQId) of
		...
		EverInfo->
			Type = everquest_db:get_type(EverProtoInfo),
			get_rewards(Type,EverProtoInfo,EverInfo)	
	end.
								...
								EverQuestFreeSetTimes ->
									case lists:keyfind(EverQuestId, 1, EverQuestFreeSetTimes) of
										...
										{EverQuestId, FreeSetTime} ->
											RecoverIntervalMax - ((?NOW - FreeSetTime) rem RecoverIntervalMax)
									end
							end
					end
			end
		...
		EverProtoInfo ->
			case everquest_db:get_free_recover_interval(EverProtoInfo) of
				...
				RecoverIntervalMax ->
					case get(everquest_free_set_times) of
						undefined ->
							put(everquest_free_set_times, [{EverQuestId, ?NOW}]),
							FreeTimes;
						[] ->
							put(everquest_free_set_times, [{EverQuestId, ?NOW}]),
							FreeTimes;
						EverQuestFreeSetTimes ->
							case lists:keyfind(EverQuestId, 1, EverQuestFreeSetTimes) of
								false ->
									put(everquest_free_set_times, [{EverQuestId, ?NOW} | EverQuestFreeSetTimes]),
									FreeTimes;
								{EverQuestId, FreeSetTime} ->
									if FreeTimes =:= 0 ->
										   lists:keyreplace(EverQuestId, 1, EverQuestFreeSetTimes, {EverQuestId, ?NOW}),
