...
training_heartbeat(BaseValue)->
	case is_in_training_map() of
		true->
			...
			Factor = get_addation_rate()+1+GlobalRate,
			AddExp = erlang:trunc(BaseExp*Factor),
			AddSp =  erlang:trunc(BaseSp*Factor),
			AddVipExp = erlang:trunc(BaseExp*VipAdd),
			AddVipSp =  erlang:trunc(BaseSp*VipAdd),
            role_op:obtain_soulpower([{soulpower, AddSp}, {vip_soulpower, AddVipSp}]),
            case role_op:obtain_exp([{expr, AddExp}, {vip_expr, AddVipExp}]) of
				level_up->
					Mylevel = get_level_from_roleinfo(get(creature_info)),
