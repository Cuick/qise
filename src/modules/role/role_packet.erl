...
handle(#role_shift_pos_c2s{posx = PosX, posy = PosY}, RolePid) ->
	role_processor:shift_pos_request(RolePid, PosX, PosY);

handle(#role_pull_objects_c2s{skill_id = SkillId, objects = Objects}, RolePid) ->
	role_processor:pull_objects_request(RolePid, SkillId, Objects);

handle(#role_push_objects_c2s{skill_id = SkillId, objects = Objects}, RolePid) ->
	role_processor:push_objects_request(RolePid, SkillId, Objects);
...
encode_get_instance_log_s2c(Ids,Times,Type, Exp, EntrustTime)->
	login_pb:encode_get_instance_log_s2c(#get_instance_log_s2c{instance_id = Ids,times = Times,type=Type,exp=Exp,entrusttimes=EntrustTime}).
...
encode_role_shift_pos_s2c(RoleId, OldPosX, OldPosY, PosX, PosY) ->
	login_pb:encode_role_shift_pos_s2c(#role_shift_pos_s2c{role_id = RoleId,
		sposx = OldPosX, sposy = OldPosY, posx = PosX, posy = PosY}).

encode_role_pull_objects_s2c(SelfId, SkillId, PullInfo) ->
	login_pb:encode_role_pull_objects_s2c(
		#role_pull_objects_s2c{role_id = SelfId, skill_id = SkillId, pull_info = PullInfo}).

encode_role_push_objects_s2c(SelfId, SkillId, PushInfo) ->
	login_pb:encode_role_pull_objects_s2c(
		#role_push_objects_s2c{role_id = SelfId, skill_id = SkillId, push_info = PushInfo}).

encode_skill_cooldown_reset_s2c() ->
	login_pb:encode_skill_cooldown_reset_s2c(
		#skill_cooldown_reset_s2c{}).

encode_query_travel_server_status_s2c(Status) ->
	login_pb:encode_query_travel_server_status_s2c(
		#query_travel_server_status_s2c{status = Status}
		).