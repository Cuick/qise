...
-export([create_role/9,create_role_rpc/9]).
...
get_pet_skill(RoleInfo)->
	element(#roleattr.pet_skill,RoleInfo).
put_pet_skill(RoleInfo,NewValue)->
	erlang:setelement(#roleattr.pet_skill, RoleInfo, NewValue).
...
get_spouse(RoleInfo)->
	element(#roleattr.spouse,RoleInfo).
put_spouse(RoleInfo,NewValue)->
	erlang:setelement(#roleattr.spouse, RoleInfo, NewValue).

%%save
async_write_roleattr(RoleInfo)->
	{RoleId,Account,Name,Sex,Class,Level,Exp,Hp,Mana,Gold,Gift,Silver,BoundSilver,Mapid,Pos,Bufflist,Debufflist,Packagesize,Finished_quest,Guildid,Maxpet,Pet,Offline,SoulPower,StallName,Honor,FightForce,Spouse,Pet_skill} = RoleInfo, 
	TableName = db_split:get_owner_table(?ROLE_TABLE_BASE, RoleId),
	dmp_op:async_write(RoleId,{TableName,RoleId,Account,Name,Sex,Class,Level,Exp,Hp,Mana,Gold,Gift,Silver,BoundSilver,Mapid,Pos,Bufflist,Debufflist,Packagesize,Finished_quest,Guildid,Maxpet,Pet,Offline,SoulPower,StallName,Honor,FightForce,Spouse,Pet_skill}).
...
create_role_rpc(AccountId,AccountName,RoleName,Gender,ClassId,CreateIp,ServerId,Pf,Flag)->
	case node_util:get_dbnode() of
		undefined-> [];
		DbNode-> 
				case rpc:call(DbNode, ?MODULE, create_role, [AccountId,AccountName,RoleName,Gender,ClassId,CreateIp,ServerId,Pf,Flag]) of
					 {failed,Reason}-> {failed,Reason};
					 {ok,Result}-> {ok,Result};
					 _Any->{failed,?ERR_CODE_CREATE_ROLE_INTERL}
				 end
	end.	
create_role(AccountId,AccountName,RoleName,Gender,ClassId,CreateIp,ServerId,Pf,Flag)->
	case get_roleid_by_name(RoleName) of
		{ok,[]}->
			role_create_deploy:create(AccountId,AccountName,RoleName,Gender,ClassId,CreateIp,ServerId,Pf,Flag);
		{ok,T}-> slogger:msg("t: ~p~n", [T]),{failed,?ERR_CODE_ROLENAME_EXISTED}
	end.
