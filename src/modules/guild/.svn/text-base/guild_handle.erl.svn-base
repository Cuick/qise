...
handle_guild_create_c2s(Name, Type)->
	case travel_battle_util:is_travel_battle_server() of
		false ->
			...
			case guild_util:is_have_guild() of
		      ...
		      case Type of
		        1 ->
					    guild_facility:falility_required_check(?GUILD_FACILITY,1);
					  0 ->
		          role_op:check_money(?MONEY_BOUND_SILVER, Silver);
		        _ -> ?ERROR_UNKNOWN
		      end;
				_->
					?GUILD_ERRNO_ALREADY_IN_GUILD
			end,
			if
				AllCheck->
		      guild_op:create(Name, Type, Silver);
				true->
...
handle_guild_member_contribute_c2s(MoneyType,MoneyCount) when is_integer(MoneyCount), MoneyCount > 0 ->
	Myid = get(roleid),
	HasGuild = guild_util:is_have_guild(),
	HasRight = guild_util:is_have_right(?GUILD_AUTH_CONTRIBUTION),
	if
		HasGuild, HasRight ->
			guild_op:contribute(?MONEY_SILVER,MoneyCount);
		true ->
			nothing
	end;
handle_guild_member_contribute_c2s(_, _) ->
	nothing.
