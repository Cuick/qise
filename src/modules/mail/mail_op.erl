...
-export([mail_status_query_c2s/0,
		 ...
		 mail_delete_c2s/1,
		 new_notify/2]).
-export([gm_send/7,gm_send_by_roleid/7,gm_send_multi/6,gm_send_multi_2/6,gm_send_multi_with_gold/7,
	gm_send_multi_2_with_gold/7,on_playeronline/0,auction_send_by_playeritems/7,gm_send_with_gold/8,gm_send_special_equi_rpc1/5]).
										   ...
										   case check_max_mail_count(ToId) of
											...
											case dal:write_rpc(MailObject) of
												{ok}->
													...
													mail_sucess(),
													slogger:msg("aaaaaaaaaaaaaaaaaaaaaaaaaToId:~p,Title:~p,Content:~p~n",[ToId,Title,Content]),
													gm_logger_role:role_send_mail(get(roleid),ToId,MailId,LostItemsTmpAndCount,Add_Silver,0,Title,Content),
													{ok};
												_Any-> 
...
gm_send_multi_2(FromName,ToName,Title,Content,TemplateIds,Add_Silver) ->
    if length(TemplateIds) > 3 ->
            {A, B} = lists:split(3, TemplateIds),
            gm_send_multi(FromName,ToName,Title,Content,A,Add_Silver),
            gm_send_multi_2(FromName,ToName,Title,Content,B,0);
        true ->
            gm_send_multi(FromName,ToName,Title,Content,TemplateIds,Add_Silver)
    end.

gm_send_multi_with_gold(FromName,ToName,Title,Content,TemplateIds,Add_Silver,Add_Gold)->
	case role_db:get_roleid_by_name_rpc(ToName) of
		[]-> {failed,?ERRNO_MAIL_NO_ROLE};
		[ToId]->
			timer_center:start_at_process(),
			...
			MailObject = #mail{mailid= MailId,
							   ...
							   send_time=Now,
							   type=?MAIL_TYPE_SYSTEM},
			gm_mail_log(MailId,FromName,ToName,ToId,Title,Content,TemplateIds,Add_Silver,Add_Gold),
			case dal:write_rpc(MailObject) of
				{ok}-> new_notify_rpc(MailId,ToId),{ok};
				_Any->{failed,?ERRNO_MAIL_INTERL}
			end;
...
gm_send_multi_2_with_gold(FromName,ToName,Title,Content,TemplateIds,Add_Silver, Add_Gold) ->
    if length(TemplateIds) > 3 ->
            {A, B} = lists:split(3, TemplateIds),
            gm_send_multi_with_gold(FromName,ToName,Title,Content,A,Add_Silver,Add_Gold),
            gm_send_multi_2_with_gold(FromName,ToName,Title,Content,B,0,Add_Gold);
        true ->
            gm_send_multi_with_gold(FromName,ToName,Title,Content,TemplateIds,Add_Silver,Add_Gold)
    end.
...
gm_send_special_equi_rpc(FromName,ToName,Title,Content,TemplateIds)->	
	case role_db:get_roleid_by_name_rpc(ToName) of
		[]-> {failed,?ERRNO_MAIL_NO_ROLE};
		[ToId]->
			timer_center:start_at_process(),
			...
			{Hi,Low} = mailid_generator:gen_newid(),
			MailId = #mid{midlow=Low,midhigh=Hi},
			[ItemIds,ItemIds1] = case TemplateIds of
						  []->[];
						  Items->
							  lists:foldl(fun({TemplateId,Count,Enchantments,Sockets,Enchant},[Acc,Acc1])->
												  if 
													  erlang:length(Acc) < 3->
														  [Acc ++ [items_op:obtain_from_gm_mail_send1(ToId,?MAIL_SLOT,TemplateId,Count,Enchantments,Sockets,Enchant)],Acc1 ++ [{TemplateId,Count}]];
													  true->
														  [Acc,Acc1]
												  end
										  end, [[],[]], Items)
			...
			MailObject = #mail{mailid= MailId,
							   ...
							   send_time=Now,
							   type=?MAIL_TYPE_SYSTEM},
			gm_mail_log(MailId,FromName,ToName,ToId,Title,Content,ItemIds1,0,0),
			case dal:write_rpc(MailObject) of
				{ok}-> new_notify_rpc(MailId,ToId),{ok};
				_Any->{failed,?ERRNO_MAIL_INTERL}
			end;
...
gm_send_special_equi_rpc1(FromName,ToName,Title,Content,TemplateIds) ->
    if length(TemplateIds) > 3 ->
            {A, B} = lists:split(3, TemplateIds),
            gm_send_special_equi_rpc(FromName,ToName,Title,Content,A),
            gm_send_special_equi_rpc1(FromName,ToName,Title,Content,B);
        true ->
            gm_send_special_equi_rpc(FromName,ToName,Title,Content,TemplateIds)
    end.