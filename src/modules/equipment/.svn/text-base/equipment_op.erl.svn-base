...
equipment_riseup(Equipment,Riseup,Protect,LuckySolts)->
	case travel_battle_op:is_in_zone() of
		false ->
			riseup(Equipment, Riseup, Protect, LuckySolts);
		true ->
...
%% 											achieve_op:achieve_update({inlay}, [get_template_id_from_iteminfo(EquipProp)], StoneLevel),
											...
											gm_logger_role:role_enchantments_item(get(roleid),EquipTemplateId,inlay,NewSocketsInfo,get(level)),
                                            broadcast_op:equipment_inlay(EquipTemplateId, SockNum);
										true->
...
equipment_stone_remove(Equipment,SockNum)->
    case get_item_from_proc(Equipment) of
        []->
            Errno = ?ERROR_EQUIPMENT_NOEXIST;
        EquipProp->
                            ...
                            _OK ->
                                case role_op:auto_create_and_put(StoneTempId, 1, removestone) of
                                    ...
                                    {ok,_} ->
                                        ...
                                        role_fighting_force:hook_on_change_role_fight_force(),
                                        EquipTemplateId = get_template_id_from_iteminfo(EquipProp),
                                        gm_logger_role:role_enchantments_item(get(roleid),EquipTemplateId,remove,NewSocketsInfo,get(level)),
                                        role_op:async_write_to_roledb(),
                                        items_op:save_to_db(),
                                        Message = equipment_packet:encode_equipment_stone_remove_s2c(),
                                        role_op:send_data_to_gate(Message)
...
equipment_stonemix(StoneList)->
	...
	{_,StoneSlot,_} = lists:last(StoneList),
	StoneInfo = get_item_from_proc(StoneSlot),
	EquipTmpId = get_template_id_from_iteminfo(StoneInfo),
    RandomRate = 
	...
	if  Result1=:=ok ->
		 ...
		 case ResultStoneQuality >= ?STONE_BROADCAST_QUALITY of
			...
			gm_logger_role:role_enchantments_item(get(roleid),EquipTmpId,hecheng,ResultStoneId_ErrNo,get(level)),
		    Msg = equipment_packet:encode_equipment_stonemix_s2c(ResultStoneId_ErrNo),
			role_op:send_data_to_gate(Msg),
	        broadcast_op:equipment_stonemix(ResultStoneId_ErrNo);
		true->
			gm_logger_role:role_enchantments_item(get(roleid),EquipTmpId,hecheng_failed,NumRequire,get(level)),
...
equipment_stonemix_batch(StoneSlot,NumRequire,NumMix)->
	...
	case enchantments_db:get_stonemix_rateinfo(NumRequire) of 
	   	[]->
	   		?CARDINAL;
		StonemixRateinfo ->
			StonemixRateinfo#stonemix_rateinfo.rate % element(#stonemix_rateinfo.rate,Stonemix_rateinfo)
	end,		
	...
	EquipTmpId = get_template_id_from_iteminfo(StoneItemInfo),
	case ResultStoneIds1 of
		[] ->
			Result = 0;
		_ ->
			[Result|_] = ResultStoneIds1
	end,
	role_op:consume_item_buff_do(),  
	gm_logger_role:role_enchantments_item(get(roleid),EquipTmpId,hecheng,Result,get(level)),
	% gm_logger_role:role_enchantments_item(get(roleid),EquipTmpId,hecheng,ResultStoneIds1,get(level)),
...
equipment_stonemix_public(StoneList,RandomRate)->
	...
	StoneMixInfo = lists:foldl(fun({_,StoneSlot,Count},{IsBondList,TotleCount,StoneType,Flag,StoneSlotInfoList})->
								case get_item_from_proc(StoneSlot) of
									[] ->
										{IsBondList , TotleCount, StoneType, Flag, StoneSlotInfoList};
									#item_info{
										isbonded = IsBonded, 
										socket_type = SocketType
									} = _StoneInfo ->
										if StoneSlotInfoList =:= []->
											StoneSlotInfoList2 = [ {StoneSlot, Count} | StoneSlotInfoList];
										true->
											case lists:keyfind(StoneSlot, 1, StoneSlotInfoList) of
													false->
														StoneSlotInfoList2 = [{StoneSlot,Count}|StoneSlotInfoList];
													{_,OldCount}->
														StoneSlotInfoList2 = lists:keyreplace(StoneSlot,1,StoneSlotInfoList,{StoneSlot,Count+OldCount})
											end
										end,

										if 	StoneType =/=0 andalso SocketType =:= StoneType ->
												{[IsBonded|IsBondList],Count+TotleCount,SocketType,true,StoneSlotInfoList2};
											StoneType =/= 0 ->
												{[IsBonded|IsBondList],Count+TotleCount,SocketType,false,StoneSlotInfoList2};
											true ->
											{[IsBonded|IsBondList],Count+TotleCount,SocketType,true,StoneSlotInfoList2}
										end
								end
							...
							case EnchantInfo of
								...
								EnchantProp->
									...
									case lists:member(TmpId, PropIdsDB) of
										false->
											io:format("TempId is ~p~n",[{TmpId,PropIdsDB,Invent}]),
										...
										true->
											...
											case equipment_db:get_enchant_property_opt_infos(Invent) of
												...
												PropInfos->
													Errno=[],											
													OldEnchants=if length(LockArr)>0 ->Enchants;true->[] end,
													BackProps = get_property_by_count(OldEnchants,PropInfos,PropertyCount,LockArr,Type),
		...
		true->
			case equipment_db:get_enchant_property_opt_infos(Invent) of
				...
				PropInfos->
					...
					if
						IsQualityRange->
                            BackPropNames = lists:map(fun({PropName, _}) ->
                                                              PropName
                                                      end, BackProps),
                            PropInfos2 = lists:filter(fun(PropInfo) ->
                                                              PropName = equipment_db:get_info_property(PropInfo),
                                                              case lists:member(PropName, BackPropNames) of
                                                                  true ->
                                                                      false;
                                                                  false ->
                                                                      true
							...
							ExtremeLy = get_property_by_count([],PropInfos2,1,[],recast),
                            BackProps++ExtremeLy;
						true->
							BackProps
					end
...
equipment_recast_confirm(Equipment)->
	case get_item_from_proc(Equipment) of 
		[]->
			nothing;
		EquipmentProp->
...
get_property_by_count(_,_,[],LockArr,Type)->
	[];
get_property_by_count(OldEnchants,PropInfosParam,Count,LockArr,Type)->
	  PropInfos = 
			...
			EquipProp->
				case get_item_from_proc(Riseup) of
					...
					RiseupProp->
						...
						LuckyAddRate = lists:foldl(fun({PacketSlot,TotalNum},RateAcc)->
						...
						case get_item_from_proc(Protect) of
							[]->
								ProtectProp2 = [],
								PtotectPropAddSuccessProb = 0;
							ProtectProp->
								ProtectPropTemplateId = get_template_id_from_iteminfo(ProtectProp),
								case lists:member(ProtectPropTemplateId, Protectdb) of
									false->
										ProtectProp2 = [],
										PtotectPropAddSuccessProb = 0;
									true->
										%consume_item(Protect)
										ProtectProp2 = ProtectProp,
										case items_op:extra_use_condition(ProtectPropTemplateId) of
											false ->
												PtotectPropAddSuccessProb = 0;
											{ok, AddSuccessRate} ->
												PtotectPropAddSuccessProb = AddSuccessRate div 100
										end
								end
						end,
						...
						FinalSuccess = erlang:trunc(((SuccessRate + LuckyAddRate) * (1+GuildAdditong/100))+VipAddition) + PtotectPropAddSuccessProb,
						if 
							FinalSuccess >= RandomRate ->
								case ProtectProp2 =:= [] of
									true ->
										[];
									false ->
										consume_item(Protect)
								...
								case package_op:where_slot(Equipment) of
									body->
										Inventory = get_inventorytype_from_iteminfo(EquipProp),
										achieve_op:role_attr_update(),
							...
							true->
								...
								gm_logger_role:role_enchantments_item(get(roleid),EquipTemplateId,star_failed,Failure,get(level)),
								case ProtectProp2 =:= [] of
									true ->
										if 
