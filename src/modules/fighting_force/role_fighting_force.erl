...
hook_on_change_role_fight_force()->
	...
	= compute_buffers:compute(get(classid), get(level), get(current_attribute), get(current_buffer), [], [],BaseAttr,[],OtherAttr,[]),
	...
	%zhangzhizhen debug message
	%slogger:msg("player: ~p compute fight force: ~p~n DebugBuffers:~n ~p~n",[get(roleid), Fighting_force, DebugBuffers]),
