...
%% @doc 根据所有的活动定义，生成进入条件数据
create_av_msg() ->
	Fun = fun({Id, Info}, Acc) ->
		#activity_value_proto{id = Id, com_condition = ComCondition} = Info,
		case ComCondition of
			[] -> Acc;
			{Msg, Op, MaxValue} -> Acc ++ [{Msg, Op, MaxValue, Id}]
		end
	end,
	ets:foldl(Fun, [], ?AV_ETS_TABLE).

%% @doc 根据活动，生成所有活动的使用状态数据
%% [{活动id，}]
create_av_state()->
