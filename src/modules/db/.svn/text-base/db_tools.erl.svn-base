...
wait_for_all_db_tables()->	
	...
	slogger:msg("wait_tables_in_dbnode end ~n"),
	wait_for_local_ram_tables().

wait_tables_in_dbnode()->
	DbNode = node_util:get_dbnode(),
	Result = case dbmaster:is_db_prepread(DbNode) of
		true ->
			rpc:call(DbNode, ?MODULE, wait_for_all_db_tables_in_db_node, [],infinity);
		false ->
			db_load_wait
	end,
	if
		Result =:= ok ->
			ok;
		true ->
			slogger:msg("wait_for_all_db_tables ERROR ~p ~n",[Result]),
			timer:sleep(1000),
			wait_for_all_db_tables()
	end.
...
wait_for_local_ram_tables()->
	case is_need_ram_table(node()) of
		true->
			mnesia:start(),
			mnesia:change_config(extra_db_nodes, node_util:get_all_nodes()--[node()]),
			mnesia:change_table_copy_type(schema, node(), disc_copies),
			lists:foreach(fun(Table) ->
				mnesia:add_table_copy(Table, node(), ram_copies)
			end, db_operater_mod:get_all_ram_table());
		_->
			nothing
	end.
...
%% @doc 读取game_server.option中配置的节点需要同步的内存表
get_node_ram_tables(Node)->
