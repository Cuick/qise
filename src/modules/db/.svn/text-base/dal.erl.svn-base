...
-export([get_all_table/1, do_get_all_table/1, get_result/2, do_get_result/2]).
...
run_transaction(Transaction)->
	case mnesia:transaction(Transaction) of
		{aborted,Reason} -> slogger:msg("run_transaction error ~p ~n",[erlang:get_stacktrace()]),{failed,erlang:get_stacktrace()};
		{atomic, []}	 -> {ok,[]};
		{atomic, Result}-> {ok,Result}
	end.
...
write(Object)->
	...
	case mnesia:sync_transaction(WriteFun) of
		{aborted,Reason} -> slogger:msg("write error ~p Object ~p ~n",[Reason,Object]),{failed,Reason};
		{atomic, failed} -> slogger:msg("write error ~p Object ~p ~n",[failed,Object]),{failed,"read table failed when write"};
		{atomic, ok}	 -> {ok}
	end.
...
% 返回某个分拆数据库表所有表
get_all_table(TableNameBase) ->
	DbNode = node_util:get_dbnode(),
	rpc:call(DbNode, ?MODULE, do_get_all_table, [TableNameBase]).

do_get_all_table(TableNameBase) ->
	TableList = mnesia:system_info(tables),
	TableNameBaseList = atom_to_list(TableNameBase),
	TableNameBaseLength = length(TableNameBaseList),
	lists:foldl(fun(Ele, Acc) ->
				Ele2 = atom_to_list(Ele),
				case string:substr(Ele2, 1, TableNameBaseLength) =:= TableNameBaseList of
					true ->
						Acc ++ Ele;
					false ->
						Acc
...
% 从分拆表中匹配读取记录
get_result(Pattern, TableList) ->
	DbNode = node_util:get_dbnode(),
	rpc:call(DbNode, ?MODULE, do_get_result,[TableList, Pattern]).

do_get_result([], Pattern) ->
	false;
do_get_result([H|T], Pattern) ->
	case mnesia:dirty_match_object(Pattern) of
		[] ->
			do_get_result(T, Pattern);
		RecordList ->
			RecordList
	end.
