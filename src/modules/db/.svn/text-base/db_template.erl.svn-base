...
create_template_role(TemplateId,RoleName,Account,ServerId)->
	 slogger:msg("db_template create_template_role ServerId ~p Account ~p ~n",[ServerId,Account]),
	try
		...
		NewRoleQuickBar = lists:map(fun(QuickBarItem) ->
			QuickBarItem2 = erlang:setelement(1, QuickBarItem, QuickBarTable),
			erlang:setelement(#role_quick_bar.roleid, QuickBarItem2, RoleId)
		end, RoleQuickBar),
		
		NewRoleSkills = lists:map(fun(Skill) ->
			Skill2 = erlang:setelement(1, Skill, RoleSkillTable),
			erlang:setelement(#role_skill.roleid, Skill2, RoleId)
		end, RoleSkill),
		...
		QF = fun()->
					...
					lists:foreach(fun(X)-> mnesia:write(X) end, NewRoleQuickBar),
					lists:foreach(fun(X)-> mnesia:write(X) end, NewRoleSkills),
					lists:foreach(fun(X)-> mnesia:write(X) end, NewPlayerItems),
					mnesia:write(RoleQuests3)
			end,
		case dal:run_transaction(QF) of
			{ok,_}-> {ok,RoleId};
			Error-> slogger:msg("Error ~p ~n",[Error]),{failed}
		end
	
	catch
		throw : no_roleattr_template -> slogger:msg("exception:no_roleattr_template~n"),{failed};
		throw : no_role_quick_bar_template ->slogger:msg("exception:no_role_quick_bar_template~n"), {failed};
		throw : no_role_skill_template -> slogger:msg("exception:no_role_skill_template~n"),{failed};
		throw : no_playeritems_template ->slogger:msg("exception:no_playeritems_template~n"), {failed};
		throw : noquest_role_template ->slogger:msg("exception:noquest_role_template~n"), {failed};
		throw : Reason ->slogger:msg("exception:~p ~n",[Reason]), {failed};
		E:Re-> slogger:msg("exception:~p ~p ~n",[E,Re]),{failed}
	end.
