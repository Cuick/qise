...
-export([init/1,load_from_db/1,export_for_copy/0,load_by_copy/1,write_to_db/0,hook_on_online/0,hook_on_offline/0,
		 answer_sign_request_c2s/0,answer_question_c2s/3,answer_reward/2,hook_on_level_up/1, get_vip_prop_count/1]).
...
load_from_db(RoleId) ->
	case answer_db:get_answer_roleinfo(RoleId) of
		...
		{ok,AnswerRoleInfo}->
			...
			case answer_processor:get_answer_info(RoleId) of
				[] ->
					put(role_answer_info, []);
				{Double, Auto, RoleScore, RoleRank, RankList} ->
					put(role_answer_info,{get(roleid), RoleScore, 0, RoleRank, Double, Auto, []}),
					send_question_s2c(-1, RoleScore, RoleRank, 0),
					Message = answer_packet:encode_answer_sign_success_s2c(),
					role_op:send_data_to_gate(Message),
					case RankList =/= [] of
						true ->
							MessageRankList = lists:map(fun({_, Rname, Rscore, _})->
										{aqrl, Rname, Rscore}
								end, RankList),
							Message2 = answer_packet:encode_answer_question_ranklist_s2c(MessageRankList),
...
apply_answer_question_right(RoleId,Id,Flag,Continu,Double,Auto,NewAnsweredList)->
	case answer_processor:apply_answer_question({RoleId, Id, Flag, Double, Auto}) of
		{Score,Rank}->
								...
								false->
									...
									if
										...
										true->
											if
												Flag =:= 1 ->
													Errno = apply_answer_question_wrong(RoleId,Id, -1, 0, Double - 1, Auto, NewAnsweredList);
												true ->
													Errno = apply_answer_question_wrong(RoleId,Id,-1,0,Double,Auto,NewAnsweredList)
											end
									end;
