...
path_find_by_range(Begin,End,Range)->
	{X1,Y1} = Begin,
	{X2,Y2} = get_random_pos_around(End),
...
get_random_pos_around({X, Y}) ->
	PosAround = [{X-1, Y+1}, {X, Y+1}, {X+1, Y+1}, {X-1, Y}, {X+1, Y}, {X-1, Y-1}, {X, Y-1}, {X+1, Y-1}],
	PosValid = lists:filter(fun(Pos) ->
		mapop:check_pos_is_valid(Pos,get(map_db))
	end, PosAround),
	PosList = PosValid ++ [{X, Y}],
	lists:nth(random:uniform(length(PosList)), PosList).
