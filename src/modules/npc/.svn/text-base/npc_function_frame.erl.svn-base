...
%% Exported Functions
%%
-export([do_action/5,do_enum/4,add_function/3,list_npc_function/3,list_everquest_response/1,
	do_action_without_check/5,add_npc_function/1,delete_npc_function/1]).
	...
	Code = case check_state(RoleMapId, RolePos, NpcInfo) of
		...
		true ->
			...
			case lists:keymember(?NPC_FUNCTION_QUEST,#kl.key,OriResponse) of
				true->
					{Quests,QuestStatus} = list_quest_response(NpcId);
				_->
					Quests = [],
					QuestStatus = []
			end,
			...
			Response = lists:foldl(fun(Item, ResponseTmp) ->
				{_,FunctionKey,_} = Item,
			...
			if
				Response =:= [], Quests =:= [], QuestStatus=:= [], EverQuests=:= [] ->
					nothing;
				true ->
...
list_quest_response(NpcId)->
	...
	CreatureInfo->
		...
		case quest_op:calculate_questgiver_state(AccList,ComList) =/= ?QUEST_STATUS_UNAVAILABLE of
			true->
				QuestStatuList = quest_op:calculate_questgiver_details(AccList,ComList),
...
proc_do_action(RoleMapId,RoleInfo,NpcId,FunctionId,FunctionInput)->
	case get_npc_function_info(NpcId,FunctionId) of
	   ...
	   {_,{ActionM,ActionF,ActionA},_}->
	       % slogger:msg("yanzengyan, in npc_function_frame:proc_do_action, ActionM: ~p, ActionF: ~p, ActionA: ~p, FunctionInput: ~p~n", [ActionM, ActionF, ActionA, FunctionInput]),
		   try
...
add_npc_function(FunctionInfo) ->
	NpcId = FunctionInfo#npc_functions.npcid,
	...
	end,FunctionInfo#npc_functions.function).

delete_npc_function(FunctionInfo) ->
	NpcId = FunctionInfo#npc_functions.npcid,
	lists:foreach(fun(FunctionId) ->
			delete_function_from_ets(NpcId, FunctionId)
	end, FunctionInfo#npc_functions.function).