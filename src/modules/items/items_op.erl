...
-export([obtain_from_mail_by_itemids/1,lost_from_mail_by_itemids/2,
			...
			obtain_from_gm_mail_send/4, add_new_item_by_info/1,obtain_from_gm_mail_send1/7]).
...
create_objects_with_ownerid1(Slot,TemplateId,Count,OwnerId,CoolDownArgh,Enchantments,SocketsInfo,Duration)->
		TemplateInfo = item_template_db:get_item_templateinfo(TemplateId),
		BondType = item_template_db:get_bonding(TemplateInfo),
		Id = itemid_generator:gen_newid(),
		%%处理绑定
		...
		OverDueInfo = create_item_overdue(TemplateInfo),
		ChentInfo = item_template_db:get_enchant_ext(TemplateInfo), 
		BaseInfo = create_item_baseinfo(Id,OwnerId,TemplateId,Enchantments,Count,Slot,IsBond ,SocketsInfo,Duration,CoolDownInfo,ChentInfo,OverDueInfo),
		FullInfo_Noduration = set_protoinfo_to_iteminfo(BaseInfo,TemplateInfo),
		FullInfo = set_duration_to_iteminfo(FullInfo_Noduration,get_maxduration_from_iteminfo(FullInfo_Noduration)),
		if
			Slot < ?SLOT_PACKAGE_ENDEX->
...
obtain_from_gm_mail_send(RoleId,Slot,TemplateId,Count)->
	...
	Enchant = get_enchant_from_iteminfo(ItemsInfo),
	OverdueInfo = get_overdueinfo_from_iteminfo(ItemsInfo),
	playeritems_db:mail_add_playeritems(ItemId,RoleId,TemplateId,Enchantments,Count,Slot,Isbonded,Sockets,Duration,CoolDownInf,Enchant,OverdueInfo),
	ItemId.

obtain_from_gm_mail_send1(RoleId,Slot,TemplateId,Count,Enchantments,Sockets,Enchant)->
	{ItemId,ItemsInfo} = items_op:create_objects_with_ownerid1(Slot,TemplateId,Count,RoleId,[],Enchantments,Sockets,Enchant),
	Isbonded = get_isbonded_from_iteminfo(ItemsInfo),
	Duration = get_duration_from_iteminfo(ItemsInfo),
	CoolDownInf = get_cooldowninfo_from_iteminfo(ItemsInfo),
	OverdueInfo = get_overdueinfo_from_iteminfo(ItemsInfo),
...
% 物品额外使用条件
extra_use_condition(ItemTemplateId) ->
	ItemTemplateInfo = item_template_db:get_item_templateinfo(ItemTemplateId),
	UseConditionTupleList = item_template_db:get_use_condition(ItemTemplateInfo),
	case lists:keyfind(consume_gold, 1, UseConditionTupleList) of
		false ->
			case lists:keyfind(add_success_rate, 1, UseConditionTupleList) of
				false ->
					false;
				{add_success_rate, AddSuccessRate} ->
					% 增加成功率
					{ok, AddSuccessRate}
			end;
		{consume_gold, Num} ->
			% 花费金币
			case role_op:check_money(?MONEY_GOLD, Num) of
				false ->
					Msg = pet_packet:encode_send_error_s2c(?ERROR_LESS_GOLD),
					role_op:send_data_to_gate(Msg),
					false;
				true ->
%					role_op:money_change(?MONEY_GOLD, -Num, use_item),
					{ok, Num}
			end
	end.

add_new_item_by_info(ItemInfo) ->
	ItemId = get_id_from_iteminfo(ItemInfo),
	put(items_info, [{ItemId, ItemInfo, {{0,0,0},0}, 1} | get(items_info)]).