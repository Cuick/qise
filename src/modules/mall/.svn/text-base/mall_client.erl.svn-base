...
add_friend(Name)->
	Term = #add_friend_c2s{target_name=Name},
...
equipment_stone_remove(Equip,SockNum)->
    Term = #equipment_stone_remove_c2s{equipment=Equip,socknum=SockNum},
    Binary = login_pb:encode_equipment_stone_remove_c2s(Term),
    sendtoserver(Binary).

equipment_stonemix(Stone)->
	Term = #equipment_stonemix_c2s{stonelist=Stone},
	Binary = login_pb:encode_equipment_stonemix_c2s(Term),
	sendtoserver(Binary).
...
handle_info({auth,AccountName,UserId},State)->
	...
	AuthTerm = #user_auth_c2s{username=AccountName},
...
handle_info(#delete_friend_success_s2c{fn=Fname},State) ->
...
handle_info(#achieve_init_s2c{parts=Parts},State) ->
	io:format("mall_client:achieve_init_s2c,~p~n",[Parts]),
	{noreply,State};
handle_info(#achieve_update_s2c{part=Part},State) ->
