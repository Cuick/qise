...
check_expired_sales_item(OldSalesItems) ->
	{MegaSec,Sec,_} = timer_center:get_correct_now(),
    CurSec = MegaSec*1000000 + Sec,
	{NowSaleItems, ExpireItems} = lists:foldl(
		fun(Info, {NowSaleItems, ExpireItems}) ->
            Id = mall_item_db:get_id_from_iteminfo(Info),
            Uptime = mall_item_db:get_uptime_from_iteminfo(Info),
            Duration = mall_item_db:get_duration_from_iteminfo(Info),
            if 
            	CurSec>=(Uptime+Duration*?MINITE)->
	                ...
	                {NowSaleItems, [Info | ExpireItems]};
	            true ->
	                {[Info | NowSaleItems], ExpireItems}
            end
    end, {[], []}, OldSalesItems),
    {NowSaleItems, ExpireItems}.
...
flush_sales_item()->
	FlushList = case mall_item_db:get_mallinfo_by_sales_type(0) of
		[]->
			random_sales_item([],[],[]);
		OldSalesItems->
			{NowSaleItems, ExpiredItem} = check_expired_sales_item(OldSalesItems),
			random_sales_item(OldSalesItems, NowSaleItems, ExpiredItem)
	end,
	flushfun(FlushList).
...
random_sales_item(OldSalesItems, NowSaleItems, ExpiredItems)->
	...
	if
		NeedFlushCount>0->
			InTimeList = get_intime_list(NowSaleItems,NeedFlushCount),
			InTimeLen = erlang:length(InTimeList),
			if
				InTimeLen >= NeedFlushCount->
					InTimeList;
				true->
					NoTimeList = get_random_notime_list(NowSaleItems,NeedFlushCount-InTimeLen),
...
get_random_notime_list(NowSaleItems,Count)->
	CheckFun = fun(Info)->
		Id = mall_item_db:get_id_from_iteminfo(Info),
		IsInOldList = lists:keymember(Id, 2, NowSaleItems),
		if
			IsInOldList->
				false;
			true->
...
make_checktime_list(TmpSalesTime)->
	...
	case lists:keyfind(2, 1, TmpSalesTime) of
		...
		{_,{StartTime,EndTime}}->
			...
			SaleStartSecs = ServerStartSecs + StartTime * ?ONE_MINUTE_SECOND,
			SaleEndSecs = ServerStartSecs + EndTime * ?ONE_MINUTE_SECOND,
			SaleStartTime = calendar:gregorian_seconds_to_datetime(SaleStartSecs),
			SaleEndTime = calendar:gregorian_seconds_to_datetime(SaleEndSecs),		
...
get_intime_list(NowSaleItems,Count)->
					...
					if 
						(LimitTime =:= -1) or (LimitTime > CurTime) ->
							...
							case role_op:check_money(MoneyType, CostMoney) of
								true->
                                    MoneyChangeReason = atom_to_list(lost_mall) ++ "|" ++ integer_to_list(MitemId) ++ "|" ++ integer_to_list(Count),
									case role_op:money_change(MoneyType, -Money*Count,list_to_atom(MoneyChangeReason)) of
										ok ->
											quest_op:update({mall_item,MitemId},Count),
											RoleInfo = get(creature_info),
											RoleId = get_id_from_roleinfo(RoleInfo),
											gm_logger_role:role_buy_mall_item(RoleId,MitemId,Money,Count,MoneyType,get_level_from_roleinfo(RoleInfo)),
											role_op:auto_create_and_put(MitemId, Count, Reason),
											if LimitCount >= Count ->
												   case BuyType of 
														?BUY_MALL_ITEM->
															mall_item_db:update_mall_item_rpc_call(MitemId,Count);
														?BUY_SALES_ITEM->
															mall_item_db:update_sales_item_rpc_call(MitemId,Count)
													end;
												true->
													nothing
											end,
											case MoneyType of
												?MONEY_GOLD->
                                                    ...
                                                    broadcast_op:buy_item(MitemId, Money);
												_->
													nothing
											end,
											broadcast_op:specify_item(MitemId),
