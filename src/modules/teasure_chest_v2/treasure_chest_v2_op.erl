		...
		Times=<ReMainSize->												%% packet is enough
			if  
				ConsumeType =:= ?GOLD_CONSUME->      					%% consume gold
				    Gold =lists:nth(BeadType,treasure_chest_v2_db:get_need_consume_gold(Times)),
				    case role_op:check_money(?MONEY_GOLD, Gold) of
					    true->											%% gold is enough
						    role_op:money_change(?MONEY_GOLD, -Gold, treasure_chest_cost),
...
lottery_operate(BeadType,Times,BindTimes,NonBindTimes,ConsumeType)->
    top_bar_manager:hook_activity_count(?ACTIVITY_LOTTERY, Times),
    % broadcast_op:treasure_count(Times),
	[BindProtoId,NonBindProtoId] = treasure_chest_v2_db:get_protoid(BeadType),
	Level = get(level),
	Class = get(classid),
...
broad_obtain(ProtoId,Count)->
    ...
    %% 因广播过频导入游戏卡，先屏掉。
    %% broadcast_op:treasure_chest(ProtoId, Count).
