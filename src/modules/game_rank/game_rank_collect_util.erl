...
gather(RoleId,Info,RankList,RankTopList,RankFreshList,CollectList,RankType,LostList)->
				...
				true->
					...
					if
						Length >= ?COLLECT_LIST_MAX_NUM ->
							refresh_gather(RankList,RankTopList,RankFreshList,CollectList,RankType,LostList);
						true->
							nothing
					end;
				_->
...
refresh_gather(RankList,RankTopList,RankFreshList,CollectList,RankType,LostList)->
	...
	if
		RankType =/= []->
			...
			NewLostList = lists:foldl(fun({RoleIdTemp,_,_}, Acc)->
			...
			put(LostList, NewLostList);
		true->
			ignor
	end,
    erlang:apply(game_rank_manager_op:get_module_by_type(RankType),after_gather, []).
