...
proc_debuff_skill(CreatureInfo, TargetInfo, SkillInfo)->
	TargetID= creature_op:get_id_from_creature_info(TargetInfo),
	SelfID = creature_op:get_id_from_creature_info(CreatureInfo),
	{TargetResult2, AoeResult2} = case skill_db:get_isaoe(SkillInfo) =:= 1 of
			false ->				%%单体攻击
	...
	TargetType = skill_db:get_target_type(SkillInfo),
	{SelfDamage, SelfBuffer} = case TargetType =:= ?SKILL_TARGET_SELF_ENEMY of
		true ->
			%% 伤害包括自己,不计算miss和暴击,计算self伤害和释放者buff
			{calculate_normal_damage(self,CreatureInfo,TargetInfo,SkillInfo),
				skill_db:get_caster_buff(SkillInfo)};
		false ->
			{0, []}
	end,
	SelfResult = case is_self_immune_skill(CreatureInfo) of
		true ->
			if
				SelfDamage =/= 0; SelfBuffer =/= [] ->
					[{SelfID, {normal,SelfDamage}, SelfBuffer}];
				true ->
					if
						RefectDamage =/= 0 ->
							[{SelfID, immune, []}];
		...
		false ->
			SelfDamage2 = SelfDamage + RefectDamage,
			if
				SelfDamage2 =/= 0; SelfBuffer =/= [] ->
					[{SelfID, {normal,SelfDamage2}, SelfBuffer}];
				true ->
					[]
			end
	end,
	SelfResult ++ TargetResult2 ++ AoeResult2.

%%return: {Damage,BuffList}, Damage = missing/{critical,CRD}/{normal,Dmg}
damage_process(Type,CreatureInfo, TargetInfo, SkillInfo) ->	
	case is_self_immune_skill(TargetInfo) of
		false ->
			do_damage_process(Type, CreatureInfo, TargetInfo, SkillInfo);
		true ->
			{immune, []}
	end.
...
is_buff_immunes(AttackerInfo, TargetInfo,{{BuffId,BuffLevel},BuffHitRate})->
	...
	case is_self_immune_ctrl(TargetInfo) andalso Resisttype >= 1 andalso Resisttype =< 5 of
		true ->
			true;
		false ->	
			...
			%%基数为100	BuffHitRate [0,1000] BuuferImmues [0,1000]
			BuffRateEndRate = erlang:trunc(BuffHitRate*(100 - Factor)*(1000 - BuuferImmues)),
...
is_missing(AttackerInfo, TargetInfo,Addtoin) ->
	...
	RandomRange = erlang:max(HitValue,400),    %% 400/1000 = 40%
...
%%	AttackerId = creature_op:get_id_from_creature_info(AttackerInfo),
    ...
    case AttackType of
   	...
   	Damage = ( ((10000+erlang:max(Critical-Toughness,0))/10000)*RealAttack - Defence )*(90+random:uniform(20))*(1000 - Immune)*SkillRatio/100000 + BaseDamage,
   	case is_pvp(AttackerInfo, TargetInfo) of
   		true -> TrueDamage = erlang:trunc(erlang:max(Damage*0.8,util:even_div(RealAttack*(1+random:uniform(7))/100,1))); 		%%PVP
   		false->	TrueDamage = erlang:trunc(erlang:max(Damage,util:even_div(RealAttack*(2+random:uniform(8))/100,1)))
   	end,
...
is_self_immune_ctrl(SelfInfo) ->
	lists:member(immune_ctrl, creature_op:get_extra_state_from_creature_info(SelfInfo)).

is_self_hold(SelfInfo) ->
	lists:member(hold, creature_op:get_extra_state_from_creature_info(SelfInfo)).

is_self_immune_skill(SelfInfo) ->
	lists:member(immune_skill, creature_op:get_extra_state_from_creature_info(SelfInfo)).
