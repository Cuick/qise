...
dragon_fight_stop(BuffInfo,Faction)->
	put(is_dragon_fighting,false),
	Anser_time = get(starttime),
	{MegaSecs, Secs, _MicroSecs} = timer_center:get_correct_now(),
	CurSec = Secs+MegaSecs*1000000,
	Result = dragon_fight_processor:get_user_result(get(roleid)),
	gm_logger_role:insert_log_activity(get(roleid),1,Faction,Result,0,0,Anser_time,CurSec),
	quest_special_msg:proc_specail_msg(quest_special_msg:dragon_fight_end()),
	role_op:remove_buffer(BuffInfo).
