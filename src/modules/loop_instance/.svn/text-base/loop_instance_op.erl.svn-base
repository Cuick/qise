...
proc_client_msg(#entry_loop_instance_apply_c2s{type = Type})->
	...
	try
		case travel_battle_op:is_in_zone() of
			true ->
				throw(?TRAVEL_BATTLE_HAS_REGISTERED);
			_ ->
				nothing
		end,
		...
		VoteMsg = loop_instance_packet:encode_entry_loop_instance_vote_s2c([LeaderVoteInfo|VoteInfos]),
...
% 添加类型和经验
% [{type, times, type2, exp, count}]
get_instance_record2()->
	TodayTimeRecord = get(loop_instance_record),
	case TodayTimeRecord of
		[]->
			[];
